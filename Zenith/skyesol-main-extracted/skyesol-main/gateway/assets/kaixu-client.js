...
(function(global){
  ...
  function headersWithAuth(h, apiKey){
  ...
  async function streamChat({ apiBase, apiKey, payload, headers, onMeta, onDelta, onDone, onError, onEvent, opts }){
    ...
    const h = headersWithAuth(headers, apiKey);
  ...
  async function submitJob({ apiBase, apiKey, payload, headers }){
    const base = getApiBase(apiBase);
  ...
  async function autoChat(args){
    ...
    if (forced === "job") {
      const j = await submitJob({ apiBase:a.apiBase, apiKey:a.apiKey, payload, headers:a.headers });
      if (a.onMeta) a.onMeta({ mode:"job", job_id: j.job_id || j.id, submit: j });
      const r = await pollJob({ apiBase:a.apiBase, apiKey:a.apiKey, job_id: j.job_id || j.id, headers:a.headers, onStatus:a.onStatus, opts:a.opts });
    ...
    if (!useJob) {
      try {
        await streamChat({
          ...
          onMeta:a.onMeta, onDelta:a.onDelta, onDone:a.onDone, onError:a.onError, onEvent:a.onEvent, opts:a.opts
        });
        return { ok:true, mode:"stream" };
      } catch (e) {
