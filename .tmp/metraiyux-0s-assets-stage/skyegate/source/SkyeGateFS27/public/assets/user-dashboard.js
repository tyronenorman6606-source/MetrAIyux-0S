(function () {
  ...
  function setResetUI(isVisible) {
    const panel = $("#userResetPanel");
  ...
  async function authJson(path, body, token = "") {
  ...
  async function finishEmailLogin(email, password) {
    store.apiBase = ($("#userApiBase")?.value || "").trim();
    const data = await authJson("/.netlify/functions/auth-login", { email, password });
    if (data.password_reset_required || data.reset_required) {
      passwordResetState = {
        email,
      ...
      setResetUI(true);
      showToast("Set a new password to finish login.", false);
      return;
    }
    if (!data.session?.token) throw new Error("Login succeeded but no session token was returned.");
  ...
  $("#userEmailLoginBtn")?.addEventListener("click", async () => {
    const email = ($("#userEmail")?.value || "").trim();
    const password = ($("#userPassword")?.value || "").trim();
    if (!email || !password) return showToast("Enter email and password.", false);
    try {
      await finishEmailLogin(email, password);
    } catch (error) {
      store.token = "";
      showToast(error.message, false);
      setAuthUI(false);
  ...
  $("#userResetPasswordBtn")?.addEventListener("click", async () => {
    try {
      if (!passwordResetState?.token) throw new Error("Reset session expired. Log in again with the temporary password.");
      const next = ($("#userNewPassword")?.value || "").trim();
      const confirm = ($("#userNewPasswordConfirm")?.value || "").trim();
      if (!next || !confirm) throw new Error("Enter and confirm the new password.");
      if (next !== confirm) throw new Error("New passwords do not match.");
      await authJson("/.netlify/functions/auth-change-password", {
        current_password: passwordResetState.currentPassword,
        new_password: next
      }, passwordResetState.token);
      const email = passwordResetState.email;
      passwordResetState = null;
      $("#userPassword") && ($("#userPassword").value = next);
      $("#userNewPassword") && ($("#userNewPassword").value = "");
      $("#userNewPasswordConfirm") && ($("#userNewPasswordConfirm").value = "");
      showToast("Password changed. Opening dashboard.", true);
      await finishEmailLogin(email, next);
    } catch (error) {
      showToast(error.message, false);
    }
  });
