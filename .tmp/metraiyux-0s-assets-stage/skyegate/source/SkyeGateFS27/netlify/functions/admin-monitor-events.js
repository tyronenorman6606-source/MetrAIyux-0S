...
export default wrap(async (req) => {
  ...
  const after_id = clampInt(url.searchParams.get("after_id"), 0, 0, 1_000_000_000);
  ...
  const next_after_id = events.length ? events[events.length - 1].id : after_id;

  return json(200, { events, next_after_id }, cors);
});
