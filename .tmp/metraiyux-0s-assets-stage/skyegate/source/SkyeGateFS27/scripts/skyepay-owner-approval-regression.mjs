...
assert.equal(routex.owner_approval_required, true, "RouteX must hold paid status for owner-approved activation");
assert.equal(routex.activation_path, "owner_approved_after_route_scope");
...
assert.equal(skyePayOfferRequiresOwnerApproval(starter), true, "Starter app lane should hold paid status for owner-approved activation");
...
assert.equal(metadata.activation_path, "owner_approved_after_route_scope");
