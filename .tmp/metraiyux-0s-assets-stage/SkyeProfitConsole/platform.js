(() => {
  ...
  const runtimeName = () => RUNTIME_BASE.includes("/api/profit") ? "0S Worker runtime" : "same-folder runtime";
  ...
  function renderRuntime() {
    ...
    label.textContent = state.runtime.online ? "Runtime live" : "Runtime pending";
  ...
  async function pushCloseBriefToRuntime(brief) {
    try {
      ...
      addProof("runtime_close_brief_archived", `${brief.label} close brief was archived into the ${runtimeName()}.`, "runtime");
  ...
  async function pushPackToRuntime(pack) {
    try {
      ...
      state.runtime.online = true;
      addProof("runtime_review_pack_archived", `${pack.label} was archived into the ${runtimeName()}.`, "runtime");
