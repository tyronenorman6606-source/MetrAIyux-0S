(() => {
  ...
  function fromStorage() {
    ...
    for (const key of LEGACY_KEYS) {
      ...
      if (tokenLooksValid(token)) return { ...(parsed || {}), token, source: parsed?.source || key, client: parsed?.client || "SkyeGate session" };
    }
    ...
    overlay.innerHTML = `
      <div class="profit-gate-card">
        ...
        <label class="profit-gate-field">
          ...
          <input id="skyeProfitGateToken" type="password" autocomplete="off" placeholder="0S should already be signed in; fallback only">
        </label>
        <div class="profit-gate-actions">
          <button class="void-button primary" id="skyeProfitGateUnlock" type="button">Attach Fallback Session</button>
