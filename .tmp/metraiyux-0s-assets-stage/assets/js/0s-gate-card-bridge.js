(() => {
  ...
  const VERSION = "2026-05-20.0s-gate-card-bridge";
  const SESSION_KEYS = [
  ...
  function normalizeCards(inputCards, claims = {}) {
    ...
    const defaults = [
      ...
      ["omega", "Omega Runner Security", "omega-control"]
    ];
    defaults.forEach(([id, name, scope]) => {
  ...
  function normalizeSession(input = {}, source = "unknown") {
    ...
    const platformId = input.platform_id || input.platformId || input.platform || "metraiyux-0s";
    const usageLane = input.usage_lane || input.usageLane || input.lane || "0s-gate-card";
    const normalized = {
      token,
  ...
  function sessionFromStorage() {
    for (const key of SESSION_KEYS) {
      ...
      if (parsed) {
        const normalized = normalizeSession(parsed, parsed.source || key);
        if (normalized) return normalized;
      }
      const raw = readStore(sessionStorage, key) || readStore(localStorage, key);
      const normalized = parseMaybeSession(raw, key);
      if (normalized) return normalized;
    }
    return null;
  }
  ...
  function headers(extra = {}) {
    ...
    return {
      ...
      "x-skye-usage-lane": session.usage_lane || "0s-gate-card",
  ...
  function installFetchPatch(options = {}) {
    ...
    globalThis.fetch = (input, init = {}) => {
      ...
      Object.entries(headers({
        ...
        "x-skye-usage-lane": options.usageLane || current()?.usage_lane || "0s-gate-card"
      })).forEach(([key, value]) => {
