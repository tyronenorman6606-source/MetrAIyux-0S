(function(){
  ...
  const minimizeStorageKey = `metraiyux.runtimeTruth.minimized:${surfaceId}`;
  let runtimeTruthMinimized = false;
  ...
  function storedMinimized(){
    try{
      return window.localStorage?.getItem(minimizeStorageKey) === '1';
    }catch(_err){
      return false;
    }
  }

  function setStoredMinimized(value){
    runtimeTruthMinimized = Boolean(value);
    try{
      window.localStorage?.setItem(minimizeStorageKey, runtimeTruthMinimized ? '1' : '0');
    }catch(_err){
      // Ignore storage failures; the visible button should still work for this page load.
    }
  }
  ...
  function renderTruth(){
    ...
    el.id = 'osRuntimeTruth';
    el.className = 'os-runtime-truth';
    el.dataset.osRuntimeTruth = 'true';
    el.dataset.minimized = runtimeTruthMinimized ? 'true' : 'false';
    el.setAttribute('aria-label', '0S runtime truth');
    el.innerHTML = runtimeTruthMinimized ? `
      <button class="os-runtime-truth__minibar" type="button" data-os-runtime-expand aria-label="Expand 0S runtime truth for ${escapeHtml(record.name || surfaceId)}">
        <span class="os-runtime-truth__minibar-copy">
          <strong>${escapeHtml(record.name || surfaceId)}</strong>
          <span>${escapeHtml(runtimeState.health.label)}</span>
        </span>
        <span class="os-runtime-truth__badge ${cls}">${escapeHtml(badge)}</span>
        <span class="os-runtime-truth__toggle-icon" aria-hidden="true">+</span>
      </button>
    ` : `
      <div class="os-runtime-truth__inner">
        <div class="os-runtime-truth__head">
          ...
          <div class="os-runtime-truth__actions">
            <span class="os-runtime-truth__badge ${cls}">${escapeHtml(badge)}</span>
            <button class="os-runtime-truth__toggle" type="button" data-os-runtime-minimize aria-label="Minimize 0S runtime truth for ${escapeHtml(record.name || surfaceId)}">Minimize</button>
          </div>
        </div>
    ...
    el.querySelector('[data-os-runtime-minimize]')?.addEventListener('click', () => {
      setStoredMinimized(true);
      renderTruth();
    });
    el.querySelector('[data-os-runtime-expand]')?.addEventListener('click', () => {
      setStoredMinimized(false);
      renderTruth();
    });
  ...
  installFetchMonitor();
  ready(async () => {
    runtimeTruthMinimized = storedMinimized();
    try{
