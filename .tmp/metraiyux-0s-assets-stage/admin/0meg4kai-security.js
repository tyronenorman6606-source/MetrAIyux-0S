...
const OmegaKAI = (()=>{
  ...
  function scan(text){
    const gate=bridge()?.requireSession?.({platformId:'0meg4kai-security',usageLane:'security-review'})
      || bridge()?.current?.();
    ...
    const receipt={id:'omega_'+Date.now(), created_at:new Date().toISOString(), input:text, decision, findings, route: decision==='allow_customer_scoped' ? 'customer_workspace_brain' : '0meg4kAI_to_admin_approval', gate: gate ? {source:gate.source, actor:gate.actor, platform_id:gate.platform_id, cards:gate.gate_cards?.map(card=>card.id)||[]} : {source:'missing'}, skyerunners_route: decision==='allow_customer_scoped' ? 'skyerunner-brain-sync' : 'skyerunner-bug-hunter', notes:[
    ...
    bridge()?.record?.('0meg4kai_scan', {decision, findings:findings.map(f=>f.id), route:receipt.route, skyerunners_route:receipt.skyerunners_route}, gate || undefined);
