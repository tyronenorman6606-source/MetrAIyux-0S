...
async function vaultUnlock(passphrase){
  ...
  toast('Vault unlocked', 'success', '0S/SkyGate AI is enabled. Workspace encryption may apply.');
...
function humanizeBlockReason(reason){
  ...
  if (r.includes('auth') || r.includes('token') || r.includes('key')) return '0S/SkyGate authentication failed or this session is not entitled.';
  return null;
}
...
function activeGatewayHeaders(){
  const bearer = activeGatewayBearer();
  return {
    'Content-Type':'application/json',
    ...(gateBridge()?.headers?.({
      'x-skye-platform':'kaixu-codestudio',
      'x-skye-usage-lane':'ai-code-studio'
    }) || {}),
...
async function callKaixu(userText){
  if (!activeGatewayBearer()){
    toast('0S gate required', 'warn', 'Sign into 0S/SkyGate to use AI.');
    renderAssistantStatus('0S/SkyGate session required.');
    return;
  }
  if (!state.online){
    toast('Offline', 'warn', 'AI requires connectivity to the 0S/SkyGate AI gateway.');
  ...
  const headers = activeGatewayHeaders();
...
function renderAssistantStatus(text = null){
  ...
  else s.textContent = state.aiBusy ? 'Working…' : (activeGatewayBearer() ? '0S/SkyGate ready.' : '0S/SkyGate session required.');
}
...
function renderVaultUnlockModal(){
  ...
  modalBody.appendChild(el('div',{class:'small', text:'Unlock only if this workspace is encrypted locally. AI auth comes from the active 0S/SkyGate session.'}));
  modalBody.appendChild(el('div',{class:'hr'}));
  modalBody.appendChild(pass);
  modalBody.appendChild(el('div',{class:'hr'}));
  modalBody.appendChild(el('div',{class:'row'},[
...
const PLATFORM_BACKEND_STORAGE_KEY = 'kaixu_platform_backend_base';
const PLATFORM_BACKEND_PIN_KEY = 'kaixu_platform_backend_base_pinned';
const PLATFORM_LOCAL_BACKEND_DEFAULT = 'http://localhost:7137';
const PLATFORM_0S_BACKEND_BASE = '/api/kaixu-codestudio';

function platformDefaultBackendBase(){
  const configured = window.METRAIYUX_API_BASES?.kaixuCodestudio;
  if (configured) return configured;
  if (/^(localhost|127\.0\.0\.1)$/i.test(location.hostname)) return PLATFORM_LOCAL_BACKEND_DEFAULT;
  return PLATFORM_0S_BACKEND_BASE;
}

function platformStoredBackendBase(){
  const stored = localStorage.getItem(PLATFORM_BACKEND_STORAGE_KEY);
  const pinned = localStorage.getItem(PLATFORM_BACKEND_PIN_KEY) === '1';
  if (stored && (pinned || stored !== PLATFORM_LOCAL_BACKEND_DEFAULT || /^(localhost|127\.0\.0\.1)$/i.test(location.hostname))) return stored;
  return platformDefaultBackendBase();
}

function platformBackendUrl(path){
  const raw = String(path || '');
  if (/^https?:\/\//i.test(raw)) return raw;
  const base = platformBackendBase();
  if (/^https?:\/\//i.test(base)) return base + raw;
  const cleanBase = String(base || platformDefaultBackendBase()).replace(/\/+$/, '');
  if (cleanBase === PLATFORM_0S_BACKEND_BASE && raw.startsWith('/api/')) return cleanBase + raw.slice('/api'.length);
...
function defaultPlatform(){
  ...
  return {
    ...
    webhookInbox: [],
    receipts: [],
    backendBase: platformStoredBackendBase(),
...
function ensurePlatformState(){
  ...
  if (!state.platform.backendBase || (state.platform.backendBase === PLATFORM_LOCAL_BACKEND_DEFAULT && localStorage.getItem(PLATFORM_BACKEND_PIN_KEY) !== '1' && !/^(localhost|127\.0\.0\.1)$/i.test(location.hostname))) state.platform.backendBase = platformStoredBackendBase();
...
function platformBackendBase(){
  ensurePlatformState();
  return String(state.platform.backendBase || platformStoredBackendBase()).replace(/\/+$/, '');
}

async function platformSetBackendBase(value){
  ensurePlatformState();
  state.platform.backendBase = String(value || platformDefaultBackendBase()).trim().replace(/\/+$/, '');
  localStorage.setItem(PLATFORM_BACKEND_STORAGE_KEY, state.platform.backendBase);
  localStorage.setItem(PLATFORM_BACKEND_PIN_KEY, '1');
...
async function platformBackendFetch(path, options={}){
  ...
  const res = await fetch(platformBackendUrl(path), {...options, headers});
  const text = await res.text();
  let payload;
  try { payload = text ? JSON.parse(text) : {}; } catch(e){ payload = {raw:text}; }
  if (!res.ok){
...
function renderBackendBridge(){
  ensurePlatformState();
  const wrap = el('div',{class:'platformStack'});
  const input = el('input',{class:'input', value:state.platform.backendBase || platformDefaultBackendBase(), autocomplete:'off', 'aria-label':'Backend base URL'});
  ...
  wrap.appendChild(el('div',{class:'small', text:'0S uses /api/kaixu-codestudio as the same-domain proof adapter. Use http://localhost:7137 after running npm run platform:server for the full local provider engine.'}));
  if (health){
    const ok = !!health.ok;
  ...
  const steps = el('div',{class:'list'},[
    ...
    el('div',{class:'listItem'},[
      el('div',{class:'left'},[
        ...
        el('span',{text:'Create/unlock an optional encrypted workspace vault. AI auth follows the 0S/SkyGate session.'})
      ]),
