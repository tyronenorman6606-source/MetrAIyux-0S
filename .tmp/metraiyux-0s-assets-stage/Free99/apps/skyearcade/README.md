...
- Local-first gameplay with no app-local login or required backend.
...
The SEO and AI-discovery files use this canonical production origin:

```text
https://skyearcade.skyesoverlondon.com
```

When deploying to a different live domain, replace that origin everywhere it appears in:
...
Identity belongs to the parent platform lane. For upstream sync, the app exposes a local bridge:
