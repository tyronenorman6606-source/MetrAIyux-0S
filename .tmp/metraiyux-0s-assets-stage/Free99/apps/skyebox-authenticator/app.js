    ...
    (() => {
      ...
      const APP_VERSION = '3.1.0-recovery';
      ...
      const els = {
        ...
        resetLocalVaultBtn: $('#resetLocalVaultBtn'),
      ...
      function bindEvents() {
        ...
        els.recoveryImportFile?.addEventListener('change', importVaultFile);
        els.changePasswordBtn.addEventListener('click', openPasswordModal);
        els.wipeVaultBtn.addEventListener('click', wipeLocalVault);
        els.resetLocalVaultBtn?.addEventListener('click', resetLocalVaultFromLock);
      ...
      function renderAuthState() {
        ...
        els.resetLocalVaultBtn?.classList.toggle('hidden', !hasVault);
      ...
      function resetLocalVaultFromLock() {
        if (!state.vaultRecord) {
          toast('No local vault is stored in this browser.');
          return;
        }
        const phrase = prompt('Type RESET SKYBOX to remove the locked local vault from this browser. This does not touch exported backups.');
        if (phrase !== 'RESET SKYBOX') return;
        stopScanner();
        localStorage.removeItem(STORE_KEY);
        sessionStorage.removeItem('skyebox.pendingLegacyImport');
        state.accounts = [];
        ...
        renderAuthState();
        renderAccounts();
        toast('Locked browser vault reset. Import a backup or create a new vault.');
      }
