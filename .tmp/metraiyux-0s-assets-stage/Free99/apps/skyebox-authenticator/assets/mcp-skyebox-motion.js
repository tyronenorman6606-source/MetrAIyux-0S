(() => {
  ...
  function createScopedAnimate(options = {}) {
    const { scope, reduceMotion: reduceMotion2 } = options;
    function scopedAnimate(subjectOrSequence, optionsOrKeyframes, options2) {
      ...
      if (isSequence(subjectOrSequence)) {
        ...
        animations = animateSequence(subjectOrSequence, reduceMotion2 !== void 0 ? { reduceMotion: reduceMotion2, ...sequenceOptions } : sequenceOptions, scope);
      } else {
        const { onComplete, ...rest } = options2 || {};
        if (typeof onComplete === "function") {
          animationOnComplete = onComplete;
        }
        animations = animateSubject(subjectOrSequence, optionsOrKeyframes, reduceMotion2 !== void 0 ? { reduceMotion: reduceMotion2, ...rest } : rest, scope);
      }
  ...
  // metraiyux_0s_site/Free99/apps/skyebox-authenticator/src/mcp-skyebox-motion.js
  ...
  function mountMotionChrome() {
    const progress2 = ensureElement("div", "skyebox-motion-progress", "skyebox-motion-progress");
    document.body.prepend(progress2);
    scroll((_progress, { y }) => {
      progress2.style.transform = `scaleX(${Math.max(0, Math.min(1, y?.progress || 0))})`;
    });
  ...
  function mountLivingBackground() {
    ...
    const draw = (time2 = 0) => {
      ...
      for (const p of state.particles) {
        ...
        const alpha2 = 0.12 + Math.sin(t * 1.5 + p.phase) * 0.06;
        ctx.fillStyle = p.hue === 0 ? `rgba(238,193,97,${alpha2})` : p.hue === 1 ? `rgba(69,221,255,${alpha2})` : `rgba(149,116,255,${alpha2})`;
        ctx.beginPath();
