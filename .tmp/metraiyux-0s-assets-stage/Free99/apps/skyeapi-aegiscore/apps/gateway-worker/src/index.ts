...
export interface Env {
  ...
  SKYE_TRUST_0S_GATE_HEADERS?: string;
}
...
function trust0sGateHeaders(env: Env): boolean {
  return env.SKYE_TRUST_0S_GATE_HEADERS !== "false";
}

function gateSessionToken(request: Request): string {
  const explicit = request.headers.get("x-0s-gate-session") || request.headers.get("x-skye-gate-session") || "";
  const bearer = request.headers.get("authorization")?.match(/^Bearer\s+(.+)$/i)?.[1] || "";
  return String(explicit || bearer || "").trim();
}

function gateCardsFromRequest(request: Request): string[] {
  const raw = [
    request.headers.get("x-0s-gate-cards"),
    request.headers.get("x-skye-gate-cards"),
    request.headers.get("x-0s-gate-card"),
    request.headers.get("x-skye-gate-card")
  ].filter(Boolean).join(",");
  return raw.split(/[,|\s]+/).map((item) => item.trim().toLowerCase()).filter(Boolean);
}

function gateProjectId(request: Request): string {
  return String(
    request.headers.get("x-skye-project-id") ||
    request.headers.get("x-0s-project-id") ||
    request.headers.get("x-workspace-project-id") ||
    "proj_skye_default"
  ).trim();
}

function hasAcceptedGateCard(request: Request): boolean {
  const cards = gateCardsFromRequest(request);
  const count = Number(request.headers.get("x-0s-gate-card-count") || "0");
  const accepted = new Set(["0s-core", "fs27", "skyeapi-aegiscore", "aegiscore", "0meg4kai", "omega", "skyerunners"]);
  return cards.some((card) => accepted.has(card)) || (count > 0 && request.headers.get("x-0s-admin-scope") === "skyeapi-aegiscore");
}

function gateAdminAccepted(request: Request, env: Env): boolean {
  return trust0sGateHeaders(env) && Boolean(gateSessionToken(request)) && hasAcceptedGateCard(request);
}

async function gateAuthRecord(request: Request, env: Env, token: string): Promise<AuthRecord | null> {
  if (!trust0sGateHeaders(env) || !token || !hasAcceptedGateCard(request)) return null;
  const tokenHash = await sha256(`0s-gate:${token}`);
  const scopes = (request.headers.get("x-skye-scopes") || request.headers.get("x-0s-scopes") || "*")
    .split(",")
    .map((scope) => scope.trim())
    .filter(Boolean);
  return {
    projectId: gateProjectId(request),
    scopes: scopes.length ? scopes : ["*"],
    createdAt: new Date().toISOString(),
    label: "0s-skygate-session",
    keyHash: `0s-gate:${tokenHash.slice(0, 32)}`
  };
}
...
async function authenticate(request: Request, env: Env): Promise<AuthRecord | Response> {
  ...
  if (!record || record.revoked) {
    const gateRecord = await gateAuthRecord(request, env, token);
    if (gateRecord) return gateRecord;
  }
  if (!record || record.revoked) return json(request, env, { ok: false, error: "Invalid or revoked capability credential or 0S gate session." }, 401);
  if (record.expiresAt && Date.parse(record.expiresAt) <= Date.now()) {
    return json(request, env, { ok: false, error: "Expired capability credential." }, 401);
  }
  return { ...record, keyHash: tokenHash };
}

function requireAdmin(request: Request, env: Env): Response | null {
  if (env.SKYE_ADMIN_KEY && request.headers.get("x-skye-admin-key") === env.SKYE_ADMIN_KEY) return null;
  if (gateAdminAccepted(request, env)) return null;
  return json(request, env, { ok: false, error: "Unauthorized admin request. Active 0S/SkyGate admin gate card required." }, 401);
}
...
async function updateKeyLastUsed(env: Env, record: AuthRecord): Promise<void> {
  if (record.keyHash.startsWith("0s-gate:")) return;
