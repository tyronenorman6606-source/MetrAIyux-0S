...
function gateSession() {
  ...
  if (!bridge || typeof bridge.current !== "function") return null;
  return bridge.current();
}

function gateClaims() {
  return gateSession()?.claims || {};
}

function defaultGatewayUrl() {
  const claims = gateClaims();
  return String(
    claims.skyeapi_gateway_url ||
    claims.skyeapiGatewayUrl ||
    claims.aegiscore_gateway_url ||
    claims.gateway_base_url ||
    localStorage.getItem("skyeapi.console.baseUrl") ||
    ""
  ).trim().replace(/\/$/, "");
}
...
function defaultProjectId() {
  ...
  return String(
    claims.skyeapi_project_id ||
    claims.project_id ||
    claims.workspace_project_id ||
    session.project_id ||
    localStorage.getItem("skyeapi.console.projectId") ||
    "proj_skye_default"
  ).trim();
}
...
function boot() {
  gateBridge()?.installFetchPatch?.({ platformId: "skyeapi-aegiscore", usageLane: "aegiscore-console" });
...
function saveConnection() {
  state.baseUrl = ($("base-url").value.trim() || defaultGatewayUrl()).replace(/\/$/, "");
  state.projectId = $("project-id").value.trim() || defaultProjectId();
...
function renderResult(data) {
  ...
  $("result").textContent = typeof data === "string" ? data : JSON.stringify(redactForConsole(data), null, 2);
...
function redactForConsole(value) {
  ...
  return Object.fromEntries(Object.entries(value).map(([key, item]) => {
    if (/(api[-_]?key|token|secret|authorization|bearer)/i.test(key)) {
      return [key, item ? "[held-by-0S-SkyGate]" : item];
    }
    return [key, redactForConsole(item)];
  }));
}
...
function requireBase() {
  saveConnection();
  if (!state.baseUrl) throw new Error("SkyeAPI gateway route is not present in the active 0S/SkyGate session.");
}

function activeGateBearer() {
  return gateSession()?.token || "";
}

function gateHeaders(extra = {}) {
  const bridge = gateBridge();
  const headers = bridge && typeof bridge.headers === "function"
    ? bridge.headers({
      "x-skye-platform": "skyeapi-aegiscore",
      "x-0s-platform": "skyeapi-aegiscore",
      "x-skye-usage-lane": "aegiscore-console",
      "x-skye-project-id": state.projectId,
      "x-0s-project-id": state.projectId
    })
    : {};
  const bearer = activeGateBearer();
  if (bearer && !headers.authorization) headers.authorization = `Bearer ${bearer}`;
  return { ...headers, ...extra };
}

function renderGateSummary() {
  const target = $("gate-session-summary");
  if (!target) return;
  const session = gateSession();
  if (!session) {
    target.textContent = "No 0S/SkyGate session detected. Open this console from the 0S Gate card.";
    return;
  }
  const cards = (session.gate_cards || []).map((card) => card.id || card.name).filter(Boolean);
  target.textContent = `${session.actor || "0S operator"} · ${cards.length || 0} gate cards · project ${state.projectId}`;
}

async function request(path, options = {}) {
  requireBase();
  const headers = gateHeaders({ ...(options.headers || {}) });
  if (options.admin) headers["x-0s-admin-scope"] = "skyeapi-aegiscore";
...
async function importEnv() {
  return run("import-env", async () => {
    ...
    gateBridge()?.record?.("skyeapi_env_imported", { platform_id: "skyeapi-aegiscore", project_id: state.projectId, capability_count: data.manifest?.capabilities?.length || 0 });
    return data;
  });
}
...
async function createKey() {
  return run("create-key", async () => {
    const data = await request(endpoints.createKey(), { method: "POST", admin: true, json: { projectId: state.projectId, scopes: scopesFromInput("key-scopes"), label: "0s-gate-entitlement", expiresAt: $("key-expires-at").value.trim() || undefined } });
    $("key-result").textContent = "Gate entitlement receipt recorded. Browser-held scoped keys are suppressed.";
    gateBridge()?.record?.("skyeapi_gate_entitlement_recorded", { platform_id: "skyeapi-aegiscore", project_id: state.projectId, scopes: scopesFromInput("key-scopes") });
    return data;
  });
}
...
async function loadCapabilities() {
  return run("capabilities", async () => {
    const data = await request(endpoints.capabilities(), { bearer: activeGateBearer() });
...
async function callCapability(dryRun) {
  return run(dryRun ? "dry-run" : "live-call", async () => {
    ...
    return request(endpoints.call(), { method: "POST", bearer: activeGateBearer(), json: { capability, input, dryRun } });
  });
}
...
async function runWorkflow(dryRun) {
  return run(dryRun ? "workflow-dry-run" : "workflow-live", async () => {
    ...
    const data = await request(endpoints.call(), { method: "POST", bearer: activeGateBearer(), json: { capability: "workflow.run", input, dryRun } });
    renderWorkflowReceipts(data);
    return data;
  });
}
...
async function exportBillingUsage(format) {
  return run(`billing-export-${format}`, async () => {
    requireBase();
    const response = await fetch(`${state.baseUrl}${endpoints.billingUsageExport(format)}`, { headers: gateHeaders({ "x-0s-admin-scope": "skyeapi-aegiscore" }) });
...
async function exportBillingInvoice(format) {
  return run(`billing-invoice-export-${format}`, async () => {
    requireBase();
    const response = await fetch(`${state.baseUrl}${endpoints.billingInvoiceExport(format)}`, { headers: gateHeaders({ "x-0s-admin-scope": "skyeapi-aegiscore" }) });
    const text = await response.text();
    if (!response.ok) throw new Error(text || `HTTP ${response.status}`);
    return text;
  });
