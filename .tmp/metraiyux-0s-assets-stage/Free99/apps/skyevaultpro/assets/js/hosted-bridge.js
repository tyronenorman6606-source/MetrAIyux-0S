window.SkyeHosted = (() => {
  ...
  const BACKUP_ADDON = {
    id: 'skyevaultpro-sovereign-backup',
    label: 'Sovereign Backup',
    priceLabel: '$4.99/mo'
  };
  ...
  function updateAccountUi() {
    ...
    const hostedButtons = document.querySelectorAll('[data-hosted-required]');
    const backupButtons = document.querySelectorAll('[data-backup-addon-required]');
    ...
    setStatus(backupAddonActive()
      ? `${BACKUP_ADDON.label} active. Hosted snapshots are available.`
      : `Signed in. Local vault remains primary; ${BACKUP_ADDON.priceLabel} ${BACKUP_ADDON.label} is required for hosted snapshots.`,
    backupAddonActive() ? 'good' : 'warn');
  }
  ...
  async function backupVault() {
    if (!backupAddonActive()) throw new Error(`${BACKUP_ADDON.label} requires the ${BACKUP_ADDON.priceLabel} add-on. Local folder sync is still available.`);
    const snapshot = await exportSnapshot();
    return api('/.netlify/functions/vault-backup', { method: 'POST', body: { snapshot, plan: BACKUP_ADDON.id } });
  }

  async function restoreVault() {
    if (!backupAddonActive()) throw new Error(`${BACKUP_ADDON.label} requires the ${BACKUP_ADDON.priceLabel} add-on. Local folder import is still available.`);
