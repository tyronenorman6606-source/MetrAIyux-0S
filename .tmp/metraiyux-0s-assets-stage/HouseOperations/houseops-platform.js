(() => {
  ...
  const PLAN_CATALOG = {
    ...
    'growth-cabinet': {
      id: 'growth-cabinet',
      name: 'Growth Cabinet with HouseOps workspace',
      setup: 3500,
      monthly: 997,
      offer: 'metraiyux-growth-cabinet',
      activation: 'SkyePay checkout creates the paid-pending 0S Growth order; FS27 owner approval activates the workspace.',
      includes: [
        '1 HouseOperations task, vendor, owner-alert, and proof workspace',
        '3 local SkyeBox vault handoffs',
        'weekly cabinet review and proof exports',
        'Relay13 workspace bridge and operator console setup path'
      ]
    },
    'autonomous-office': {
      id: 'autonomous-office',
      name: 'Autonomous Office with managed HouseOps',
      setup: 7500,
      monthly: 2497,
      offer: 'metraiyux-autonomous-office',
      activation: 'SkyePay checkout creates the paid-pending 0S Autonomous order; managed handoff remains owner-approved.',
      includes: [
        '3 HouseOperations command rooms',
        '8 local SkyeBox vault handoffs',
        'custom app-event mirror policy',
        'daily operator digest and proof review path'
      ]
    },
    'enterprise-command': {
      id: 'enterprise-command',
      name: 'Enterprise / Managed Gate custom HouseOps policy',
      setup: 15000,
      monthly: 3997,
      offer: 'metraiyux-enterprise-command',
      activation: 'Enterprise checkout or invoice starts only from written owner-approved 0S scope.',
      includes: [
        'custom HouseOperations policy',
        'custom SkyeBox custody handoff policy',
        'custom FS27 event mirror and proof exports',
        'managed gate activation boundary'
      ]
    },
    'houseoperations-standalone-review': {
      id: 'houseoperations-standalone-review',
      name: 'Standalone HouseOperations + SkyeBox review',
      setup: 0,
      monthly: 0,
      priceLabel: 'Quote only',
      setupLabel: 'Owner-approved scope required',
      checkoutPath: '../sales/pricing-offer-router.html#houseoperations-stack-included',
      activation: 'Standalone managed custody is quote-only until a managed-security custody policy and live deployment proof are approved.',
      includes: [
        'no standalone SkyePay checkout in this pass',
        'review the 0S pricing router before quoting',
        'managed custody requires separate policy and live proof'
      ]
    }
  };
  const tutorialSteps = [
  ...
  const seed = {
    ...
    proofs: [
      ...
      { id: 'pf-house-4', title: 'SkyeBox vault canonicalized', status: 'pass', at: '2026-05-17', note: 'Encrypted authenticator vault now launches from the single Free99 canonical app; HouseOperations keeps only a compatibility redirect.' },
  ...
  function skyePayUrl(offerId, client = 'metraiyux-0s') {
    ...
    url.searchParams.set('offer', offerId || PLAN_CATALOG['starter-command'].offer);
    return url.toString();
  }

  function planCheckoutUrl(plan, client = 'metraiyux-0s') {
    if (plan.checkoutPath) {
      const url = new URL(plan.checkoutPath, window.location.href);
      url.searchParams.set('client', client || 'metraiyux-0s');
      url.searchParams.set('plan', plan.id);
      return url.toString();
    }
    return skyePayUrl(plan.offer, client);
  }

  function planById(id) {
    return PLAN_CATALOG[id] || PLAN_CATALOG['starter-command'];
  }
  ...
  function activePlan(data = read()) {
    return planById(latestBillingIntent(data)?.plan_id || 'starter-command');
  }
  ...
  function kpis() {
    ...
    const activePrice = plan.priceLabel || money(plan.monthly);
    return `<div class="kpis">
      ...
      <div class="kpi"><div class="num">${esc(activePrice)}</div><div class="label">Active plan price</div></div>
    </div>`;
  }
  ...
  function planCard(plan, activeId) {
    const price = plan.priceLabel || `${money(plan.monthly)}<span>/mo</span>`;
    const setup = plan.setupLabel || money(plan.setup);
    const cta = plan.checkoutPath ? 'Review quote boundary' : 'Open approved 0S checkout';
    return `<article class="planCard ${plan.id === activeId ? 'active' : ''}">
      <p class="crumb">${esc(plan.id)}</p>
      <h3>${esc(plan.name)}</h3>
      <strong>${price}</strong>
      <p><b>Setup:</b> ${esc(setup)}</p>
      <p>${esc(plan.activation)}</p>
      <ul>${plan.includes.map((item) => `<li>${esc(item)}</li>`).join('')}</ul>
      <a class="btn" href="${esc(planCheckoutUrl(plan))}">${esc(cta)}</a>
    </article>`;
  }
  ...
  function billingForm(data = read()) {
    ...
    const optionLabel = (plan) => plan.priceLabel
      ? `${plan.name} - ${plan.priceLabel}`
      : `${plan.name} - ${money(plan.setup)} setup + ${money(plan.monthly)}/mo`;
    return `<form class="intakeForm" data-form="billing">
      <h3>Create approved plan intent</h3>
      <label>Plan<select name="plan_id">${Object.values(PLAN_CATALOG).map((plan) => `<option value="${esc(plan.id)}" ${latest?.plan_id === plan.id ? 'selected' : ''}>${esc(optionLabel(plan))}</option>`).join('')}</select></label>
      ...
      <p class="small-copy">This records the bundled 0S plan or quote-only review route. It does not fake payment; confirmed payment, quote approval, and activation live in SkyePay/FS27 and the owner-approved gate policy.</p>
  ...
  function commandHero(eyebrow, title, body, actions = '') {
    ...
    const s = stats(data);
    const plan = activePlan(data);
    const planPrice = plan.priceLabel || money(plan.monthly);
    const planUnit = plan.priceLabel ? 'review lane' : 'monthly lane';
    return `<section class="span12 commandMarquee" data-mcp-surface="radical-v3">
      ...
      <div class="missionStack" aria-label="HouseOperations live status">
        ...
        <div class="missionPlate accent"><span>Plan</span><strong>${esc(planPrice)}</strong><small>${esc(planUnit)}</small></div>
      </div>
    </section>`;
  }
  ...
  function billingView() {
    ...
    return `<section class="grid">
      ${commandHero(
        'Approved app lane',
        'Create the client handoff from the bundled 0S plan policy.',
        'HouseOperations records the current 0S plan, included scope, quote-only boundary, approved checkout route, and activation rule. Payment and entitlement enforcement stay with SkyePay and FS27.',
        `<button class="btn primary" data-action="create-billing-intent">Create Billing Intent</button><button class="btn" data-action="export-billing-intent">Export Intent</button><a class="btn" href="${esc(planCheckoutUrl(active))}">Open handoff route</a>`
      )}
      ...
      <div class="panel span12 pricingRunway"><div class="panelHead"><div><h2>Bundled Scope</h2><p>HouseOperations is sold through approved 0S plan scope today; standalone managed custody is quote-only.</p></div></div><div class="panelBody planGrid">${Object.values(PLAN_CATALOG).map((plan) => planCard(plan, active.id)).join('')}</div></div>
    </section>`;
  }
  ...
  function renderTitle(view) {
    ...
    $('#viewSub').textContent = {
      ...
      billing: 'Approved 0S plan intent, quote boundary, and activation rule.',
  ...
  function createBillingIntent(payload = {}) {
    ...
    const quoteOnly = Boolean(plan.checkoutPath);
    const intent = {
      id: uid('bill'),
      type: 'houseoperations_billing_intent',
      status: quoteOnly ? 'quote_only_owner_review' : 'ready_for_skyepay_checkout',
      created_at: new Date().toISOString(),
      plan_id: plan.id,
      plan_name: plan.name,
      ...
      checkout_label: quoteOnly ? 'Review quote boundary' : 'Open approved 0S checkout',
      owner_approval_required: true,
      activation_boundary: plan.activation,
      included_scope: plan.includes,
      proof_boundary: quoteOnly
