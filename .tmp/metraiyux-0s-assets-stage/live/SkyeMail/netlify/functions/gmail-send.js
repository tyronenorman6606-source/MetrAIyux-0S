...
function normalizeAttachments(list) {
  ...
  if (total > 18 * 1024 * 1024) {
    const err = new Error('Attachments are too large for this send lane. Keep total attachment size under 18 MB.');
    err.statusCode = 400;
    throw err;
  }
  return safe;
...
exports.handler = async (event) => {
  try {
    ...
    const replyThreadId = String(body.reply_thread_id || '').trim();
    const fromAlias = String(body.from_alias || '').trim();
    const attachments = normalizeAttachments(body.attachments || []);

    if (!to) return json(400, { error: 'Recipient is required.' });
    if (!subject) return json(400, { error: 'Subject is required.' });
    if (!text && !html) return json(400, { error: 'Message body is required.' });
