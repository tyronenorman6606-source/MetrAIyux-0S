const { createRefundIntent, appendPaymentLedger } = require('./_lib/housecircle-payment-providers');
...
exports.handler = async function(event){
  ...
  const result = await createRefundIntent({ ...body, orgId:guard.orgId }, { operatorId:guard.payload.operatorId });
  if(result.ledger){ appendPaymentLedger(guard.state, result.ledger); try{ await persistOrgState(guard.orgId, guard.state, { eventKind:'refund_intent', sourceLane:'v83-payment-provider', note:'Refund intent ledger persisted.' }); }catch(err){ return jsonResponse(503, { ok:false, error:'Refund ledger persistence failed.', detail:err.persistence || err.message }); } }
