...
```txt
metraiyux_0s_site/SkyeRouteX/workforce-command-v0.4.0/skyehands.platform.json
Dynasty-Versions/platform/user-platforms/REGISTRY.json
npm run smoke:mount
```

The mount smoke proves the local 0S SkyeHands user-platform registry points directly to this mounted app, the manifest resolves back to this source folder, and the launch/smoke profiles are ready. Do not recreate a separate Dynasty manifest folder; the canonical manifest lives with the mounted app.
