...
const expectedManifestPath = 'metraiyux_0s_site/SkyeRouteX/workforce-command-v0.4.0/skyehands.platform.json';
...
try {
  ...
  assert(row.sourceRoot === expectedSourceRoot, 'Registry sourceRoot mismatch', row);
  assert(row.manifestPath === expectedManifestPath, 'Registry manifest path mismatch', row);
  pass('registry_row_points_to_workforce_manifest', row);

  const manifestPath = path.resolve(repoRoot, row.manifestPath);
