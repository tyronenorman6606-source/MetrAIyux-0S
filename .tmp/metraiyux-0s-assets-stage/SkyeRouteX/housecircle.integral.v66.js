(function(){
  if(window.__ROUTEX_HOUSECIRCLE_V66__) return;
  window.__ROUTEX_HOUSECIRCLE_V66__ = true;

  var KEY = 'skye_routex_platform_house_circle_valuation_v66';
  var KEY_SYNC = 'skye_routex_platform_house_circle_valuation_sync_v66';
  ...
  function components(){
    return [
      ...
      { key:'productization', label:'Packaging, PWA shell, documentation, branding, investor discoverability, and operator polish', value:400000, rationale:'The app ships as a cohesive product with docs, smoke outputs, deploy guidance, and discoverable investor artifacts.' }
    ];
  }
  ...
  function buildRecord(){
    return {
      type:'skye-routexflow-codebase-valuation-v66',
      version:'66.0.0',
      ...
      summary:'This valuation reflects the entire current codebase: route and dispatch operations, proof/export infrastructure, AE/CRM layers, integrated Platform House Circle hospitality domain, live-ops mesh, cloud sync, MFA/device trust, locking, event-feed coordination, and productization/discoverability surfaces.',
      ...
      discoverability:{
        html:'./investor/SKYEROUTEXFLOW_V66_2026_ENTERPRISE_VALUATION.html',
        markdown:'./investor/SKYEROUTEXFLOW_V66_2026_ENTERPRISE_VALUATION.md',
        json:'./investor/SKYEROUTEXFLOW_V66_2026_ENTERPRISE_VALUATION.json'
      },
      notes:[
  ...
  function exportJson(){ var record = readRecord(); downloadTextFn(JSON.stringify(record, null, 2), 'skyeroutexflow_v66_valuation.json', 'application/json'); }
  function exportMarkdown(){ var record = readRecord(); downloadTextFn(markdownOf(record), 'skyeroutexflow_v66_valuation.md', 'text/markdown'); }
  function exportHtml(){ var record = readRecord(); downloadTextFn(htmlOf(record), 'skyeroutexflow_v66_valuation.html', 'text/html'); }
  ...
  async function syncValuationCloud(reason){
    ...
    var res = await fetch(basePath + '/phc-valuation', { method:'POST', headers: headers, body: JSON.stringify({ orgId: cfg.orgId, record: record, reason: reason || 'V66 valuation sync' }) });
  ...
  function openValuationModal(){
    ...
    openModalFn(
      ...
      '<button class="btn" id="hc66_json">Export JSON</button><button class="btn" id="hc66_md">Export MD</button><button class="btn" id="hc66_html">Export HTML</button><button class="btn" id="hc66_open_static">Open static report</button><button class="btn primary" id="hc66_sync">Sync valuation</button>'
    );
    var bind = function(id, fn){ var el = document.getElementById(id); if(el) el.onclick = fn; };
    bind('hc66_json', exportJson);
    bind('hc66_md', exportMarkdown);
    bind('hc66_html', exportHtml);
    bind('hc66_open_static', function(){ try{ window.open(record.discoverability.html, '_blank'); }catch(_){ exportHtml(); } });
    bind('hc66_sync', async function(){ try{ await syncValuationCloud('Manual V66 modal sync'); openValuationModal(); }catch(err){ toastFn(clean(err && err.message) || 'Valuation sync failed.', 'bad'); } });
  }

  function injectStyles(){
    if(document.getElementById('hc_v66_styles')) return;
    var style = document.createElement('style');
    style.id = 'hc_v66_styles';
    style.textContent = '.hc-v66-kpi{min-width:160px;padding:14px 16px;border:1px solid rgba(255,255,255,.12);border-radius:16px;background:rgba(255,211,106,.07)}.hc-v66-kpi .n{font-size:1.6rem;font-weight:800;color:#ffd36a}.hc-v66-kpi .d{font-size:.82rem;opacity:.82}.hc-v66-card .hint strong{color:#fff}';
  ...
  function renderValuationCard(){
    ...
    card.id = 'hc_v66_card';
    card.className = 'card hc-v66-card';
    card.innerHTML = '<h2 style="margin:0 0 8px;">2026 codebase valuation · V66</h2><div class="hint">The valuation is now shipped inside the repo and surfaced from the platform so the current build can expose its investor-facing number and artifact paths when live.</div><div class="sep"></div><div class="row" style="flex-wrap:wrap;gap:12px;"><div class="hc-v66-kpi"><div class="n">' + esc(fmtMoney(m.totalValue)) + '</div><div class="d">Current codebase valuation</div></div><div class="hc-v66-kpi"><div class="n">' + esc(String(m.components)) + '</div><div class="d">Valuation components</div></div><div class="hc-v66-kpi"><div class="n">' + esc(m.asOf) + '</div><div class="d">As-of record</div></div></div><div class="sep"></div><div class="row" style="justify-content:flex-end;flex-wrap:wrap;"><button class="btn" id="hc66_card_open">Open valuation</button><button class="btn" id="hc66_card_sync">Sync valuation</button><button class="btn primary" id="hc66_card_static">Open static report</button></div>';
    host.appendChild(card);
    var btnOpen = document.getElementById('hc66_card_open'); if(btnOpen) btnOpen.onclick = openValuationModal;
    var btnSync = document.getElementById('hc66_card_sync'); if(btnSync) btnSync.onclick = async function(){ try{ await syncValuationCloud('Quick card sync'); if(typeof render === 'function') render(); }catch(err){ toastFn(clean(err && err.message) || 'Valuation sync failed.', 'bad'); } };
    var btnStatic = document.getElementById('hc66_card_static'); if(btnStatic) btnStatic.onclick = function(){ try{ window.open('./investor/SKYEROUTEXFLOW_V66_2026_ENTERPRISE_VALUATION.html', '_blank'); }catch(_){ exportHtml(); } };
  }

  function injectToolbarButton(){
    if(document.getElementById('hc66_toolbar_btn')) return;
    var hosts = [];
  ...
  function patchRender(){
    if(window.__HC_V66_RENDER_PATCHED__) return;
    window.__HC_V66_RENDER_PATCHED__ = true;
  ...
  function bootstrap(){
    ...
    if(window.RoutexPlatformHouseCircleV64 && window.RoutexPlatformHouseCircleV64.readCloudConfig){
      try{
        ...
        if(cfg && cfg.enabled && session && session.token){ syncValuationCloud('Auto V66 valuation sync').catch(function(){}); }
      }catch(_){ }
    }
  ...
  window.RoutexPlatformHouseCircleV66 = {
