...
import { callOpenAIAudioSpeech } from '../adapters/openaiAudioSpeech'
...
import type { Env, SkySpeechRequest } from '../types'
...
export async function handleAudioSpeech(request: Request, env: Env): Promise<Response> {
  const traceId = createTraceId()
  const started = Date.now()
  const auth = await verifyAppToken(request, env)
  const body = await readJson<SkySpeechRequest>(request)
  const alias = normalizeAlias(body.engine || body.alias, 'speech')
  assertAliasAllowed(auth, alias)
  if (!isLaneEnabled(env, 'speech')) throw new KaixuError(503, 'KAIXU_LANE_DISABLED', 'This Kaixu lane is disabled or not configured.')
  const upstream = await resolveUpstreamTarget(alias, env)

  try {
    const result = await callOpenAIAudioSpeech(upstream.model, { ...body, alias }, env, alias)
    const payload = { ok: true, trace_id: traceId, engine: publicEngineName(alias), asset: result.asset, usage: result.usage }
    await insertKaixuTrace(env.DB, {
      ...
      outputSizeEstimate: estimateSize(result.asset),
