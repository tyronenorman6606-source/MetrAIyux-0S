...
export default { async fetch(req, env){
  ...
  try{
    if(path==='/api/saas/status') return json({ok:true, service:'saas-self-serve-provisioning', plans:Object.keys(PLANS), d1:!!env.SAAS_DB, kv:!!env.SAAS_KV, queue:!!env.SAAS_QUEUE, resend:!!env.RESEND_API_KEY, time:now()});
    ...
    if(path==='/api/saas/workspaces' && req.method==='POST'){
      ...
      if(env.SAAS_QUEUE) await env.SAAS_QUEUE.send({type:'workspace_provisioning', workspace_id, plan_id, services:workspace.services, at:now()});
      await audit(env,'system','create_workspace','workspace',workspace_id,b);
      await email(env,'Workspace provisioning approval needed',`<h2>Workspace pending</h2><p>Workspace ${workspace_id} for ${b.company_name||slug} is ready for approval.</p><p>Plan: ${plan_id}</p>`);
      return json({ok:true, workspace_id, slug, status:'pending_provisioning', queued:!!env.SAAS_QUEUE, persistence:env.SAAS_DB?'d1':'kv_fallback'});
    }
    ...
    if(path==='/api/saas/customer-command' && req.method==='POST'){
      const b=await body(req); const cmd_id=id('cmd'); const omega=omegaScan(b.command_text||b.command||''); const route=routeCommand(b.command_text||b.command||''); if(omega.decision==='quarantine_for_admin_review'){ route.primary='Security Gate Brain / Security QA Brain'; route.secondary='Main Automation Brain approval queue'; route.approval_required=true; } else if(omega.decision==='approval_required'){ route.secondary='Security Gate Brain / Security QA Brain'; route.approval_required=true; }
