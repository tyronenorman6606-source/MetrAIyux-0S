...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/crown/status') return Response.json({ ok:true, service:'CROWN Site Operator Brain', brains:16, persistence:{d1:!!env.CROWN_DB, kv:!!env.CROWN_KV, queue:!!env.CROWN_QUEUE}, mode:'worker-ready', time:new Date().toISOString() }, { headers:cors });
...
function classify(signal){
  ...
  const tests = [
    ...
    [['contract','legal','compliance','insurance','risk'],'Julian Mercer — Compliance Brain','Client Founder — Founder Command Brain'],
