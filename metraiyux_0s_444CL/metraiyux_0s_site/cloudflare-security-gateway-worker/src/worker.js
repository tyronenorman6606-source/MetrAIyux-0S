...
function scan(command,payload={}){
  ...
  return {decision,risk_score,findings,primary_brain:'Security Gate Brain',secondary_brain: decision==='allow_customer_scoped'?'customer_workspace_brain':'Main Automation Brain approval queue'};
}
...
async function notify(env,event){
  ...
  const res=await fetch('https://api.resend.com/emails',{method:'POST',headers:{'content-type':'application/json','authorization':`Bearer ${env.RESEND_API_KEY}`},body:JSON.stringify({from:env.RESEND_FROM_EMAIL,to:[env.ADMIN_APPROVAL_EMAIL],subject:`Security Gate Brain review: ${event.decision}`,html:`<h2>Security Gate Brain review</h2><p>Decision: <b>${event.decision}</b></p><p>Risk score: ${event.risk_score}</p><pre>${String(event.command_text||'').replace(/[<>&]/g,c=>({'<':'&lt;','>':'&gt;','&':'&amp;'}[c]))}</pre><p><a href="${link}">Open admin approval inbox</a></p>`})});
  return {sent:res.ok,status:res.status};
}
export default {async fetch(req,env){
  if(req.method==='OPTIONS') return new Response(null,{headers:CORS});
  const url=new URL(req.url);
  if(url.pathname==='/api/omega/status') return j({ok:true,service:'Security Gate Brain Security Gateway',d1:!!env.OMEGA_DB,kv:!!env.OMEGA_KV,queue:!!env.OMEGA_QUEUE,resend:!!env.RESEND_API_KEY,time:now()});
