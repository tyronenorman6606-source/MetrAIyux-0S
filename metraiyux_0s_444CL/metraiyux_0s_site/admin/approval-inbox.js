...
const ApprovalInbox = (() => {
  ...
  const approvalKey='clientCommandDeck.adminApprovals.v1';
  ...
  function renderStatus(){
    if($('approvalStatus')) $('approvalStatus').innerHTML=`<span class="status-pill">${endpoint()?'Cloudflare Worker mode':'Browser-local mode'}</span><span class="status-pill">Resend notifications supported</span><span class="status-pill">Approval gates active</span>`;
  ...
  async function loadLedger(){
    ...
    const res=await fetch(endpoint().replace(/\/$/,'')+'/api/admin/ledger'); const json=await res.json();
