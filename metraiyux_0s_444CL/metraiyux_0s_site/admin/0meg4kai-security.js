...
const OmegaKAI = (()=>{
  const key='clientCommandDeck.omeg4kai.latestScan.v1';
  const riskPatterns = [
  ...
  function scan(text){
    ...
    const receipt={id:'omega_'+Date.now(), created_at:new Date().toISOString(), input:text, decision, findings, route: decision==='allow_customer_scoped' ? 'customer_workspace_brain' : 'Security Gate Brain_to_admin_approval', notes:[
