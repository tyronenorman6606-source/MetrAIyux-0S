export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/sentinel/status') return json({ok:true, service:'Sentinel Site Operator Brain', brains:16, persistence:{d1:!!env.DB, kv:!!env.SENTINEL_LEDGER, queue:!!env.SENTINEL_QUEUE}, mode:'worker-ready', time:new Date().toISOString()});
