...
export default {
  async fetch(request, env) {
    ...
    const json = (data, status=200) => new Response(JSON.stringify(data,null,2), {status, headers:{'content-type':'application/json','access-control-allow-origin':'*','access-control-allow-methods':'GET,POST,OPTIONS','access-control-allow-headers':'content-type'}});
    if (request.method === 'OPTIONS') return json({ok:true});
    if (url.pathname === '/api/nexus/status') return json({ok:true, service:'NEXUS Site Operator Brain', brains:16, persistence:{d1:!!env.NEXUS_DB, kv:!!env.NEXUS_KV, queue:!!env.NEXUS_QUEUE}, mode:'edge-ready'});
...
function routeFor(lane){return ({lead:['Celeste Monroe Revenue Brain','Marcus Vale Operations Review'],client:['Adrian Cross Client Success Brain','Victor Saint QA Review'],candidate:['Sienna Brooks Staffing Brain','Julian Mercer Compliance Review'],vendor:['Helena Ward Vendor Brain','Julian Mercer Compliance Review'],compliance:['Julian Mercer Compliance Brain','Client Founder Founder Review'],proof:['Victor Saint QA Brain','Site Operator Brain'],finance:['Naomi Sterling Finance Brain','Marcus Vale Operations Review'],technology:['Orion Hayes Technology Brain','Site Operator Brain'],government:['Donovan Pierce Gov/Enterprise Brain','Julian Mercer Compliance Review'],founder:['Client Founder Founder Brain','Site Operator Brain'],general:['Site Operator Brain','Central Company Command Brain']})[lane] || ['Site Operator Brain','Central Company Command Brain'];}
