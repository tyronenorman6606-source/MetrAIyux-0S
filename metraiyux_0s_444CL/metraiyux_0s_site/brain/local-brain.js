...
function smartDirectAnswer(query, route, surfaces) {
  ...
  if (hasAny(q, ['skygate', 'fs27', 'auth', 'token', 'bearer', 'session', 'introspect', 'platform events', 'mirror secret', 'admin key'])) {
    return `Route this through ${primaryOwner}. FS27 is the gate layer: it validates bearer/session state through auth introspection, accepts mirrored platform events through the mirror-secret path, and keeps admin/key surfaces away from public copy. Client Command Deck should consume that gate as proof, identity, and event context, then route the work to the correct cabinet brain.`;
  }
...
function buildAnswer(query, sources) {
  ...
  return `
    <div class="answer-block serious-answer">
      ...
      <p><strong>Owner lane:</strong> ${escapeHtml(route.primary?.cabinet || 'Client Command Deck routing layer')}.</p>
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        model,
        messages: [
          { role: 'system', content: 'You are a serious Client Command Deck cabinet router. Answer with the owner brain, secondary brain, next action, and one guardrail.' },
