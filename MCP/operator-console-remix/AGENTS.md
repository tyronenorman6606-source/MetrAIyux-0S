# QuantumSkyes MCP Control Room Agent Guide

This app was scaffolded with `remix new`. Use these conventions when continuing to build it out.

This is not the deployed MCP protocol endpoint. It is the Remix 3 beta operator console for immersive MCP-backed product worlds.
...
- Keep the physical-world metaphor first. New features should map to a place, entry ritual, state, or proof action.
- Do not put owner admin codes in source. Link to the 0S owner login or exchange through a server route only once auth proxying is intentionally added.
