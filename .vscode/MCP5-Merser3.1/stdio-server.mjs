...
Usage:
  ...
  Merser31 --stdio          Start the MCP stdio server
...
Local CLI:
  npx ${MERSER_PACKAGE_NAME} --health
  npm exec --package ${MERSER_PACKAGE_NAME}@${MCP4_VERSION} -- Merser --stdio
  node /workspaces/MetrAIyux-0S/.vscode/MCP5-Merser3.1/stdio-server.mjs --stdio
  cd /workspaces/MetrAIyux-0S/.vscode/MCP5-Merser3.1 && npm start

Package:
  reserved package: ${MERSER_PACKAGE_NAME}
...
export async function runMerserCli(args = process.argv.slice(2)) {
  ...
  if (command === "--help" || command === "-h" || command === "help") {
    ...
    process.stderr.write(`Unknown Merser3.1 command: ${command}\nRun: Merser31 --help\n`);
    process.exitCode = 1;
  }
}
