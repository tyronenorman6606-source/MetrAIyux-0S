...
const ROOMS = [
  ...
  },
  {
    ...
    actions: ["mcp5_index", "mcp5_component", "mcp5_cli"],
  },
];
...
const DIMENSION_SURFACES_PER_ROOM = 4;

const ROOM_DIMENSIONS = {
  barbershop: [
    { label: "Chair Anchor", detail: "service menu", height: 0.42, width: 1.28 },
    { label: "Booking Gate", detail: "conversion drawer", height: 0.58, width: 1.38 },
    { label: "Proof Mirror", detail: "before-after rail", height: 0.72, width: 1.18 },
    { label: "Route Screen", detail: "source room", height: 0.52, width: 1.48 },
  ],
  tattoo: [
    { label: "Flash Plane", detail: "portfolio wall", height: 0.66, width: 1.34 },
    { label: "Ink Proof", detail: "artist receipts", height: 0.54, width: 1.18 },
    { label: "Consult Gate", detail: "intake surface", height: 0.74, width: 1.42 },
    { label: "Needle Map", detail: "route system", height: 0.46, width: 1.28 },
  ],
  "med-spa": [
    { label: "Treatment Orbit", detail: "modality shell", height: 0.56, width: 1.42 },
    { label: "Glow Proof", detail: "result surface", height: 0.76, width: 1.18 },
    { label: "Booking Gate", detail: "calm conversion", height: 0.48, width: 1.48 },
    { label: "Consent Screen", detail: "trust dimension", height: 0.68, width: 1.3 },
  ],
  gym: [
    { label: "Program Floor", detail: "training route", height: 0.5, width: 1.44 },
    { label: "Coach Proof", detail: "results drawer", height: 0.72, width: 1.18 },
    { label: "Member Gate", detail: "signup shell", height: 0.58, width: 1.34 },
    { label: "Session Grid", detail: "schedule plane", height: 0.46, width: 1.5 },
  ],
  "mcp-bridge": [
    { label: "Tool Registry", detail: "mcp5_index", height: 0.56, width: 1.42 },
    { label: "Prompt Vault", detail: "source packets", height: 0.76, width: 1.22 },
    { label: "CLI Gate", detail: "stdio/http", height: 0.5, width: 1.36 },
    { label: "Icon Field", detail: "component vault", height: 0.64, width: 1.26 },
  ],
};
...
const theatreProject = getProject("Merser3.1 by Skyes Over London", {
  state: {
    sheetsById: {},
    definitionVersion: "0.4.0",
    revisionHistory: ["merser3-1-runtime"],
  },
});
...
function ensureRuntime() {
  ...
  const existingRuntime = window.__MERSER31_RUNTIME__ || window.__MCP4_RUNTIME__;
  window.__MERSER31_RUNTIME__ = window.__MCP4_RUNTIME__ = existingRuntime || {
    ...
    scrollDepth: 0,
    entryDepth: 0,
    worldZoomStage: "zoomed-out",
    dimensionalSurfaces: true,
    dimensionSurfaceCount: ROOMS.length * DIMENSION_SURFACES_PER_ROOM,
    cameraDistance: 21,
    cameraStartZoomedOut: true,
    chamberPositions: clonePositions(initialPositions),
    chamberScreenPositions: {},
    startedAt: new Date().toISOString(),
  };
  window.__MERSER31_RUNTIME__ = window.__MCP4_RUNTIME__;
...
function useMcpScrollEngine(setActiveRoom, setScrollDepth) {
  useEffect(() => {
    const runtime = ensureRuntime();
    runtime.gsap = true;
    let lastDepth = -1;
    ...
    const publishScrollProgress = (progress, source = "scroll") => {
      const nextProgress = clamp(Number.isFinite(progress) ? progress : 0, 0, 1);
      const nextDepth = clamp(nextProgress * 1.18, 0, 1);
      runtime.scrollProgress = nextProgress;
      runtime.scrollDepth = nextDepth;
      runtime.entryDepth = nextDepth;
      runtime.worldZoomStage = nextDepth < 0.2 ? "zoomed-out" : nextDepth < 0.72 ? "entering" : "dimensional";
      runtime.scrollSource = source;
      runtime.cameraDistance = Number(lerp(21, 11.2, easeOutCubic(nextDepth)).toFixed(3));
      runtime.cameraScrollDriven = true;
      runtime.cameraStartZoomedOut = nextDepth < 0.18;
      runtime.surfaceRevealDepth = Number(easeOutCubic(clamp((nextDepth - 0.08) / 0.78, 0, 1)).toFixed(3));
      document.documentElement.style.setProperty("--scroll-progress", nextProgress.toFixed(4));
      document.documentElement.style.setProperty("--entry-depth", nextDepth.toFixed(4));
      if (Math.abs(nextDepth - lastDepth) >= 0.012 || nextDepth === 0 || nextDepth === 1) {
        lastDepth = nextDepth;
    ...
    lenis.on("scroll", (event) => {
      const nativeProgress = window.scrollY / Math.max(document.body.scrollHeight - window.innerHeight, 1);
      publishScrollProgress(event.progress || nativeProgress, "lenis");
    ...
    const heroTimeline = gsap.timeline({
      scrollTrigger: {
        ...
        scrub: 0.8,
        onUpdate: (self) => publishScrollProgress(self.progress, "gsap-world-entry"),
      },
    });

    heroTimeline
      .to(".world-title", { yPercent: -20, opacity: 0.46, ease: "none" }, 0)
      .to(".world-hud", { y: -34, ease: "none" }, 0)
      .to(".gate-console", { y: 38, opacity: 0.68, ease: "none" }, 0)
      .to(".entry-telemetry", { opacity: 1, y: -8, ease: "none" }, 0.12);

    publishScrollProgress(window.scrollY / Math.max(document.body.scrollHeight - window.innerHeight, 1), "initial");
...
function CameraDirector({ activeRoom, positions, theatreValues, cameraMode, cameraCommand, scrollDepth }) {
  ...
  const focusTarget = useCallback((distanceOverride) => {
    ...
    const [x, y, z] = target;
    const entry = easeOutCubic(scrollDepth);
    const directedHeight = Number(theatreValues.cameraHeight || 4.2);
    const height = lerp(10.8, directedHeight + 1.05, entry);
    const targetDistance = activeRoom === "core" ? 13.8 : 11.2;
    const startDistance = activeRoom === "core" ? 23.5 : 20.6;
    const distance = distanceOverride ?? lerp(startDistance, targetDistance, entry);
    const sideSpread = lerp(7.2, 3.6, entry);
    const side = activeRoom === "gym" || activeRoom === "barbershop" ? -1 : 1;
    controls.current.setLookAt(x + side * sideSpread, y + height, z + distance, x, y + lerp(0.85, 0.45, entry), z, true);
    runtime.cameraMoved = true;
    runtime.activeRoom = activeRoom;
    runtime.cameraMode = cameraMode;
    runtime.cameraDistance = Number(distance.toFixed(3));
    runtime.cameraHeight = Number(height.toFixed(3));
    runtime.cameraStartZoomedOut = scrollDepth < 0.18 && distance > 18;
    lastTarget.current = activeRoom;
  }, [activeRoom, cameraMode, positions, scrollDepth, theatreValues.cameraHeight]);
  ...
  useEffect(() => {
    ...
    if (cameraCommand.type === "zoom-in") focusTarget(5.8);
    else if (cameraCommand.type === "zoom-out") focusTarget(22.4);
    else focusTarget();
  ...
  useFrame(({ clock }) => {
    ...
    if (cameraMode !== "orbit360") {
      if (cameraMode === "locked") return;
      const height = lerp(10.8, directedHeight + 1.05, entry);
      const targetDistance = activeRoom === "core" ? 13.8 : 11.2;
      const startDistance = activeRoom === "core" ? 23.5 : 20.6;
      const distance = lerp(startDistance, targetDistance, entry);
      const sideSpread = lerp(7.2, 3.6, entry);
      const side = activeRoom === "gym" || activeRoom === "barbershop" ? -1 : 1;
      controls.current.setLookAt(x + side * sideSpread, y + height, z + distance, x, y + lerp(0.85, 0.45, entry), z, false);
      runtime.cameraMoved = true;
      runtime.activeRoom = activeRoom;
      runtime.cameraMode = cameraMode;
      runtime.cameraDistance = Number(distance.toFixed(3));
    ...
    const radius = lerp(activeRoom === "core" ? 18.8 : 16.2, activeRoom === "core" ? 10.2 : 8.4, entry);
    const height = lerp(10.6, directedHeight + 1.1, entry);
    ...
    runtime.cameraDistance = Number(radius.toFixed(3));
  });
...
function RoomDimensionalUniverse({ room, active, dragging, scrollDepth }) {
  const group = useRef(null);
  const surfaces = ROOM_DIMENSIONS[room.id] || [];
  const reveal = easeOutCubic(clamp((scrollDepth - 0.08) / 0.78, 0, 1));
  const activeReveal = active ? 1 : 0.36;
  const surfaceOpacity = clamp(reveal * activeReveal + (dragging ? 0.18 : 0), 0.05, 0.94);
  const baseRadius = 1.85 + (Number(room.code) % 3) * 0.12;

  useFrame((state, delta) => {
    if (!group.current) return;
    const runtime = ensureRuntime();
    group.current.visible = reveal > 0.025;
    group.current.rotation.y += delta * (0.035 + reveal * 0.045) * (active ? 1.35 : 0.72);
    group.current.rotation.x = Math.sin(state.clock.elapsedTime * 0.28 + Number(room.code)) * 0.035 * reveal;
    group.current.scale.lerp(new THREE.Vector3(lerp(0.7, active ? 1.08 : 0.94, reveal), lerp(0.7, active ? 1.08 : 0.94, reveal), lerp(0.7, active ? 1.08 : 0.94, reveal)), 0.08);
    runtime.dimensionalSurfaces = true;
    runtime.dimensionSurfaceCount = ROOMS.length * DIMENSION_SURFACES_PER_ROOM;
    runtime.activeDimensionRoom = active ? room.id : runtime.activeDimensionRoom;
    runtime.surfaceRevealDepth = Number(reveal.toFixed(3));
  });

  return (
    <group ref={group} name={`${room.title} dimensional modular surfaces`}>
      <mesh rotation={[Math.PI / 2, 0, 0]} position={[0, 0.08, 0]}>
        <torusGeometry args={[baseRadius + 0.22, 0.01, 8, 128]} />
        <meshBasicMaterial color={room.accent} transparent opacity={surfaceOpacity * 0.32} />
      </mesh>
      <mesh rotation={[1.18, 0.28, 0.74]}>
        <torusGeometry args={[baseRadius + 0.58, 0.008, 8, 128]} />
        <meshBasicMaterial color={room.color} transparent opacity={surfaceOpacity * 0.24} />
      </mesh>

      {surfaces.map((surface, index) => {
        const angle = (index / Math.max(1, surfaces.length)) * Math.PI * 2 + (Number(room.code) % 7) * 0.09;
        const orbitRadius = baseRadius + index * 0.16;
        const startRadius = orbitRadius + 2.7;
        const localReveal = easeOutCubic(clamp((scrollDepth - 0.12 - index * 0.035) / 0.68, 0, 1));
        const radius = lerp(startRadius, orbitRadius, localReveal);
        const y = lerp(1.72, 0.66 + (index % 2) * 0.7, localReveal);
        const x = Math.cos(angle) * radius;
        const z = Math.sin(angle) * radius;
        const panelOpacity = clamp(surfaceOpacity * localReveal, 0.04, 0.9);
        return (
          <group key={surface.label} position={[x, y, z]} rotation={[0.08 * (index % 2 ? -1 : 1), -angle + Math.PI / 2, 0.08 * (index - 1.5)]}>
            <Line
              points={[
                [0, -0.32, 0],
                [-x * 0.52, -y + 0.42, -z * 0.52],
              ]}
              color={index % 2 ? room.accent : room.color}
              lineWidth={active ? 1.35 : 0.72}
              transparent
              opacity={panelOpacity * 0.5}
            />
            <Float speed={0.9 + index * 0.12} rotationIntensity={0.09} floatIntensity={0.13 + index * 0.03}>
              <mesh>
                <boxGeometry args={[surface.width, surface.height, 0.05]} />
                <meshStandardMaterial
                  color={index % 2 ? room.accent : room.color}
                  emissive={room.color}
                  emissiveIntensity={active ? 0.62 : 0.32}
                  roughness={0.23}
                  metalness={0.62}
                  transparent
              ...
              <mesh position={[0, 0, 0.034]}>
                <planeGeometry args={[surface.width * 0.86, surface.height * 0.62]} />
                <meshBasicMaterial color="#020204" transparent opacity={0.42 * panelOpacity} />
              </mesh>
              <Text
              ...
              <Text
                ...
                fontSize={0.055}
                color={index % 2 ? room.color : room.accent}
                anchorX="center"
                anchorY="middle"
                maxWidth={surface.width * 0.72}
                outlineWidth={0.003}
                outlineColor="#020204"
              >
                {surface.detail}
...
function DraggableChamber({
  ...
  return (
    <group>
      ...
      >
        <RoomDimensionalUniverse room={room} active={active} dragging={dragging} scrollDepth={scrollDepth} />
  ...
  return (
    <div className="canvas-stage" data-three-world>
      <Canvas
        ...
        camera={{ position: [0, 11.5, 21], fov: 52, near: 0.1, far: 110 }}
        dpr={[1, 1.5]}
...
function RuntimePanel({ activeRoom, gateOpen, dragEvents, paused, onPauseToggle, scrollDepth }) {
  return (
    <div className="runtime-panel">
      ...
      <div className="entry-meter" aria-label="world entry depth">
        <span style={{ width: `${Math.round(scrollDepth * 100)}%` }} />
        <b>{Math.round(scrollDepth * 100)}% dimensional reveal</b>
      </div>
    ...
    <div className="world-hud">
      <header className="world-title">
        <div className="world-brand-lockup">
          ...
          <p className="eyebrow">
            <Sparkles size={15} />
            Merser3.1 by Skyes Over London
          </p>
        </div>
        <h1 className="effect-text-shimmer">Merser3.1</h1>
        <p className="world-lede">
          Start zoomed out, scroll into the source-world, then drag, orbit, inspect, search, and open the modular room dimensions.
        </p>
        ...
        <WorldControlDock
          ...
          onSelect={onSelect}
        />
        <div className="entry-telemetry" data-entry-depth={scrollDepth.toFixed(3)}>
          <span>World entry</span>
          <strong>{Math.round(scrollDepth * 100)}%</strong>
          <i>{scrollDepth < 0.2 ? "zoomed-out map" : scrollDepth < 0.72 ? "scrolling into scene" : "dimensional rooms online"}</i>
        </div>
      </header>
...
function ScrollChapters({ activeRoom, onSelect, scrollDepth }) {
  return (
    <section className="scroll-path" id="scroll-path" aria-label="Merser world scroll path">
      <div className="scroll-intro">
        ...
        <h2>Scroll from the outer map into dimensional source rooms.</h2>
        <p>
          Entry depth is live at {Math.round(scrollDepth * 100)}%. The camera starts wide, then closes in as each room unfolds into service, proof, gate, and source surfaces.
        </p>
      </div>
    ...
    <section className="source-pack-deck" id="source-packs" aria-label="Merser source-pack and CLI registry">
      ...
      <div className="source-tooling-grid">
        ...
        <article className="source-tooling-panel">
          ...
          <pre><code>{`stdio: node stdio-server.mjs --stdio
local: node /workspaces/MetrAIyux-0S/.vscode/MCP5-Merser3.1/stdio-server.mjs --stdio
http:  http://127.0.0.1:8789/mcp
live:  https://merser3-1.pages.dev/mcp
...
function App() {
  ...
  useMcpScrollEngine(setActiveRoom, setScrollDepth);
  ...
  useEffect(() => {
    ...
    runtime.drawerOpen = drawerRoom;
    runtime.scrollDepth = scrollDepth;
  }, [positions, activeRoom, gateOpen, drawerRoom, scrollDepth]);
  ...
  return (
    <main className="immersive-app" data-paused={paused ? "true" : "false"}>
      ...
      <ScrollChapters activeRoom={activeRoom} onSelect={selectRoom} scrollDepth={scrollDepth} />
