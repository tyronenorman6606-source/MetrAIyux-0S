...
const artifactRoot = resolve(repoRoot, "test-artifacts", "merser31-mcp-stress");
...
async function runPackageSmoke() {
  ...
  return {
    ...
    help: { ok: help.ok, durationMs: help.durationMs, hasUsage: help.stdout.includes("Merser31 --stdio") },
    tools: {
      ok: tools.ok,
...
async function runStdioStress() {
  ...
  for (let index = 0; index < config.stdioIterations; index += 1) {
    ...
    try {
      const response = await client.callTool({ name: "mcp5_index", arguments: {} });
      samples.push({
  ...
  try {
    ...
    const callSamples = await runPool(config.httpIterations, config.concurrency, (index) =>
      jsonFetch(`${localBase}/mcp`, {
        method: "POST",
        headers: rpcHeaders(),
        body: rpcBody(index + 1000, "tools/call", { name: "mcp5_index", arguments: {} }),
      }),
    );
    return {
...
async function main() {
  ...
  const outputPath = config.output || join(artifactRoot, `${nowIsoForPath()}-merser31-mcp-stress-report.json`);
