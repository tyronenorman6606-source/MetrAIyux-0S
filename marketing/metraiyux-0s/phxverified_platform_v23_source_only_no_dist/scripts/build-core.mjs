...
const SITE_URL = normalizeBaseUrl(process.env.SITE_URL || process.env.URL || 'https://phxverified.netlify.app');
...
function normalizeBaseUrl(url){
  ...
  return /^https?:\/\//i.test(clean) ? clean : 'https://phxverified.netlify.app';
}
...
function defaultDescription({ name, category, subcategory, city, tags }){
  ...
  return `${name} is listed in PHX Verified Network for ${service || 'local services'} in ${city || 'the Phoenix area'}. Contact the provider directly to confirm availability, scope, and pricing.`;
}
...
function ownerVerificationPage(packets){
  ...
  return pageWrap({ title:'Owner Verification | PHX Verified', description:'Owner claim and verification packet workflow for PHX Verified listings.', canonical:`${SITE_URL}/owner-verification/`, bodyClass:'owner-verification-page' }, body);
}
function lifecyclePage(queue){
  ...
  return pageWrap({ title:'Lifecycle Queue | PHX Verified', description:'Operator lifecycle and enrichment queue for seeded PHX Verified listings.', canonical:`${SITE_URL}/lifecycle/`, bodyClass:'lifecycle-page' }, body);
}
function leadRoutingPage(rules){
  ...
  return pageWrap({ title:'Lead Routing | PHX Verified', description:'Generated lead routing rules for PHX Verified buyer requests.', canonical:`${SITE_URL}/lead-routing/`, bodyClass:'lead-routing-page' }, body);
}
function opportunitiesPage(opportunities){
  ...
  return pageWrap({ title:'Opportunities | PHX Verified', description:'Category and niche opportunity index for PHX Verified operators.', canonical:`${SITE_URL}/opportunities/`, bodyClass:'opportunities-page' }, body);
}
function monetizationPage(readiness){
  ...
  return pageWrap({ title:'Monetization | PHX Verified', description:'Sellable marketplace inventory and package readiness for PHX Verified.', canonical:`${SITE_URL}/monetization/`, bodyClass:'monetization-page' }, body);
}
function apiPage(apiIndex){
  ...
  return pageWrap({ title:'Static API | PHX Verified', description:'Static JSON API endpoints generated by PHX Verified.', canonical:`${SITE_URL}/api/`, bodyClass:'api-page' }, body);
}
function fraudDefensePage(bundle){
  ...
  return pageWrap({ title:'Fraud Defense | PHX Verified', description:'Duplicate, poster-risk, and contact-fingerprint defense for PHX Verified.', canonical:`${SITE_URL}/fraud-defense/`, bodyClass:'fraud-defense-page' }, body);
}
...
function crawlControlPage(crawlBudget, report){
  ...
  return pageWrap({ title:'Crawl Control | PHX Verified', description:'Large-directory sitemap, archive, and profile-routing control for PHX Verified.', canonical:`${SITE_URL}/crawl/`, bodyClass:'crawl-page' }, body);
}
function routingControlPage(canonicalRouting, report){
  ...
  return pageWrap({ title:'Canonical Routing | PHX Verified', description:'Canonical business routing and one-posting identity map for PHX Verified.', canonical:`${SITE_URL}/routing/`, bodyClass:'routing-page' }, body);
}
function dryRunPage(dryRun){
  ...
  return pageWrap({ title:'Import Dry Run | PHX Verified', description:'Dry-run publishing safety report for PHX Verified seed imports.', canonical:`${SITE_URL}/dry-run/`, bodyClass:'dry-run-page' }, body);
}
function verificationPage(report, quality){
  ...
  return pageWrap({ title:'Verification Protocol | PHX Verified', description:'Verification, claim, suppression, and trust controls for PHX Verified business listings.', canonical:`${SITE_URL}/verification/`, bodyClass:'verification-page' }, body);
}
...
function paginatedBusinessArchivePage(page, totalPages, slice, total){
  ...
  return pageWrap({ title:`Business Archive Page ${page} | PHX Verified`, description:`Paginated PHX Verified business archive page ${page} of ${totalPages}.`, canonical:`${SITE_URL}${directoryPageRoute(page)}`, bodyClass:'business-archive-page' }, body);
}
function importHealthPage(quality, sourceBatches, posterRisk, report){
  ...
  return pageWrap({ title:'Import Health | PHX Verified', description:'Operator import quality, source batch, and poster-risk dashboard for PHX Verified.', canonical:`${SITE_URL}/import-health/`, bodyClass:'import-health-page' }, body);
}
function adminActionsPage(actionPackets, posterRisk, suppressionTemplateData){
  ...
  return pageWrap({ title:'Admin Actions | PHX Verified', description:'Operator action queue for PHX Verified duplicate control, suppression, and poster-risk review.', canonical:`${SITE_URL}/admin-actions/`, bodyClass:'admin-actions-page' }, body);
}
...
function badgeHtml(b, big = false){
  ...
  if(b.badges.mobile) list.push(['Mobile','purple']);
...
function baseHead({ title, description, canonical, type = 'website', bodyClass = '', structuredData = null, robots = '' }){
  ...
  return `<!doctype html><html lang="en"><head>
    ...
    ${sd}
  </head><body class="${html(bodyClass)}"><canvas id="sky" aria-hidden="true"></canvas><div class="grain" aria-hidden="true"></div>`;
}
function siteHeader(){
  return `<header class="topbar">
    <a class="brand" href="/" aria-label="PHX Verified home"><span class="mark"><span>PV</span></span><span><strong>PHX Verified</strong><small>Network Platform</small></span></a>
    <nav class="nav-actions" aria-label="Primary"><a href="/directory/">Directory</a><a href="/market/">Markets</a><a href="/match/">Match</a><a href="/category/">Categories</a><a href="/niche/">Niches</a><a href="/service-lanes/">Service Lanes</a><a href="/join/">Join</a><a href="/pricing/">Pricing</a><a href="/ae-command/" rel="nofollow">AE Command</a><a href="/accounts/" rel="nofollow">Accounts</a><a href="/pipeline/" rel="nofollow">Pipeline</a><a href="/activation/" rel="nofollow">Activation</a><a href="/territories/" rel="nofollow">Territories</a><a href="/kpi/" rel="nofollow">KPI</a><a href="/lead-routing/" rel="nofollow">Lead Routing</a><a href="/claims-ledger/" rel="nofollow">Claims</a><a href="/fraud-defense/" rel="nofollow">Fraud</a><a href="/owner-verification/" rel="nofollow">Owner Proof</a><a href="/coverage/" rel="nofollow">Coverage</a><a href="/import-health/" rel="nofollow">Import Health</a><a href="/api/" rel="nofollow">API</a><a href="/admin-batch/" rel="nofollow">Batch Admin</a><a href="/operator/" rel="nofollow">Seed</a></nav>
  </header>`;
}
...
function seedSchema(){
  return {
    ...
    title:'PHX Verified business seed file',
    type:'object',
    required:['businesses'],
    properties:{
...
function collectionIndexPage(collections){
  ...
  return pageWrap({ title:'Collections | PHX Verified', description:'Generated PHX Verified business collections from verification, pricing, mobile service, and request signals.', canonical:`${SITE_URL}/collection/`, bodyClass:'collection-page' }, body);
}
...
function marketIndexPage(markets, facets){
  ...
  return pageWrap({ title:'Market Matrix | PHX Verified', description:'Generated city and category market pages for PHX Verified business discovery.', canonical:`${SITE_URL}/market/`, bodyClass:'market-page' }, body);
}
...
function coveragePage(gaps, facets, report){
  ...
  return pageWrap({ title:'Coverage Gaps | PHX Verified', description:'Operator coverage gap scanner for PHX Verified city and category lanes.', canonical:`${SITE_URL}/coverage/`, bodyClass:'coverage-page' }, body);
}
function matchPage(businesses, facets){
  ...
  return pageWrap({ title:'Match Engine | PHX Verified', description:'Browser-local buyer request matching engine for seeded PHX Verified providers.', canonical:`${SITE_URL}/match/`, bodyClass:'match-page' }, body);
}
function outreachPage(outreach){
  ...
  return pageWrap({ title:'Outreach Desk | PHX Verified', description:'Operator outreach packets for PHX Verified profile enrichment and owner claims.', canonical:`${SITE_URL}/outreach/`, bodyClass:'outreach-page' }, body);
}
function sponsorPage(inventory){
  ...
  return pageWrap({ title:'Sponsor Inventory | PHX Verified', description:'Sponsored placement inventory surfaces for PHX Verified marketplace monetization.', canonical:`${SITE_URL}/sponsor/`, bodyClass:'sponsor-page' }, body);
}
function exportsPage(report){
  ...
  return pageWrap({ title:'Export Vault | PHX Verified', description:'PHX Verified export vault for JSON, CSV, VCF, match, coverage, outreach, and sponsor data.', canonical:`${SITE_URL}/exports/`, bodyClass:'exports-page' }, body);
}
...
function nicheIndexPage(businesses, facets){
  ...
  return pageWrap({ title:'Niche Index | PHX Verified', description:'Generated niche index for PHX Verified category lanes and seeded businesses.', canonical:`${SITE_URL}/niche/`, bodyClass:'niche-index-page' }, body);
}
function nichePage(niche, businesses, facets){
  ...
  if(subset.length) return directoryShell({ businesses:subset, facets, title:`${niche.name} in ${niche.category}`, eyebrow:'Niche hub', description:`Browse PHX Verified ${niche.name.toLowerCase()} businesses in the ${niche.category} lane.`, canonical:`${SITE_URL}/niche/${niche.slug}/`, routeFilter:{ niche:niche.name } });
  ...
  return pageWrap({ title:`${niche.name} | PHX Verified`, description:`PHX Verified ${niche.name} niche hub ready for seeded local businesses.`, canonical:`${SITE_URL}/niche/${niche.slug}/`, bodyClass:'niche-page' }, body);
}
function duplicateScannerPage(audit, businesses){
  ...
  return pageWrap({ title:'Duplicate Scanner | PHX Verified', description:'Duplicate posting scanner and merge ledger for PHX Verified seeded business imports.', canonical:`${SITE_URL}/duplicates/`, bodyClass:'duplicates-page' }, body);
}
function adminReviewPage(businesses, audit, report){
  ...
  return pageWrap({ title:'Admin Review | PHX Verified', description:'Admin review console for PHX Verified duplicate, suppression, and moderation signals.', canonical:`${SITE_URL}/admin-review/`, bodyClass:'admin-review-page' }, body);
}
...
function shortlistPage(businesses, facets){
  ...
  return pageWrap({ title:'Shortlist | PHX Verified', description:'Browser-local PHX Verified shortlist workspace and request packet builder.', canonical:`${SITE_URL}/shortlist/`, bodyClass:'shortlist-page' }, body);
}
function comparePage(businesses, facets){
  ...
  return pageWrap({ title:'Compare Businesses | PHX Verified', description:'Side-by-side comparison workspace for seeded PHX Verified business profiles.', canonical:`${SITE_URL}/compare/`, bodyClass:'compare-page' }, body);
}
function dealDeskPage(businesses, facets){
  ...
  return pageWrap({ title:'Deal Desk | PHX Verified', description:'PHX Verified request, claim, and new-listing operations workspace designed for upstream auth.', canonical:`${SITE_URL}/deal-desk/`, bodyClass:'deal-desk-page' }, body);
}
function auditPage(businesses, report){
  ...
  return pageWrap({ title:'Audit Queue | PHX Verified', description:'Operator data quality queue for PHX Verified seeded business records.', canonical:`${SITE_URL}/audit/`, bodyClass:'audit-page' }, body);
}
...
function homePage(businesses, facets, report){
  ...
  return pageWrap({ title:'PHX Verified Network Platform', description:'Seed-powered Phoenix business discovery platform with generated business profiles, city hubs, category hubs, and operator import tools.', canonical:`${SITE_URL}/`, bodyClass:'home-page' }, body);
}
function directoryShell({ businesses, facets, title='Business Directory', eyebrow='Live marketplace', description='Search and filter seeded Phoenix-area business listings.', canonical=`${SITE_URL}/directory/`, routeFilter = {} }){
  ...
  return pageWrap({ title:`${title} | PHX Verified`, description, canonical, bodyClass:'directory-page' }, body);
}
...
function businessPage(b, related){
  ...
  const body = `<section class="profile-hero glass"><div><a class="back-link" href="/directory/">← Directory</a><p class="eyebrow">${html(b.city)} • ${html(b.category)} • ${html(b.subcategory || 'Business profile')}</p><h1>${html(b.name)}</h1><p class="hero-text">${html(b.description)}</p>${badgeHtml(b, true)}${actionLinks(b)}</div><aside class="profile-score"><div class="score huge"><strong>${b.verification_score}</strong><small>verification score</small></div><div class="profile-mini"><span>${html(priceLabel(b))}</span><span>${html(formatDate(b.last_verified))}</span><span>${html(b.neighborhood || b.city)}</span><span>${html(b.service_area_miles ? `${b.service_area_miles} mi service area` : 'Area not listed')}</span></div></aside></section>
  <section class="profile-grid"><article class="glass section"><p class="eyebrow">Contact</p><h2>Reach ${html(b.name)}</h2><div class="contact-list">${b.phone ? `<a href="tel:${phoneDigits(b.phone)}"><strong>Phone</strong><span>${html(b.phone)}</span></a>` : ''}${b.email ? `<a href="mailto:${html(b.email)}"><strong>Email</strong><span>${html(b.email)}</span></a>` : ''}${b.website ? `<a href="${html(b.website)}" target="_blank" rel="noopener"><strong>Website</strong><span>${html(displayUrl(b.website))}</span></a>` : ''}${b.booking_url ? `<a href="${html(b.booking_url)}" target="_blank" rel="noopener"><strong>Booking</strong><span>${html(displayUrl(b.booking_url))}</span></a>` : ''}</div><div class="button-row profile-tools"><button class="btn" data-vcard='${html(JSON.stringify({ name:b.name, phone:b.phone, email:b.email, website:b.website, address:b.address, city:b.city, state:b.state, zip:b.zip, description:b.description }))}'>Download vCard</button><button class="btn" data-copy-profile>Copy profile link</button><button class="btn" data-save-business data-business-id="${html(b.id)}" data-business-name="${html(b.name)}" data-url="/business/${html(b.id)}/">Save shortlist</button><a class="btn" href="/compare/?ids=${html(b.id)}">Compare</a><a class="btn" href="/claim/?business=${html(b.id)}">Claim / update</a><a class="btn primary" href="/request/?business=${html(b.id)}">Request quote</a></div></article>
  <article class="glass section"><p class="eyebrow">Location</p><h2>${html([b.city,b.state].filter(Boolean).join(', '))}</h2><p class="muted">${html([b.address,b.neighborhood,b.city,b.state,b.zip].filter(Boolean).join(' • '))}</p><div class="map-card">${b.location ? `<span>${b.location.lat.toFixed(4)}, ${b.location.lng.toFixed(4)}</span>` : '<span>No coordinates supplied</span>'}</div>${actionLinks(b, true)}</article>
  <article class="glass section"><p class="eyebrow">Policies</p><h2>Provider signals</h2><div class="policy-list"><div><strong>Fees</strong><span>${html(b.policies.fees_transparency || b.price_note || 'Confirm directly before booking.')}</span></div><div><strong>Cancellation</strong><span>${html(b.policies.cancellation || 'Not listed')}</span></div><div><strong>Deposit</strong><span>${html(b.policies.deposit || 'Not listed')}</span></div></div></article>
  <article class="glass section"><p class="eyebrow">Hours</p><h2>Availability</h2><div class="hours">${hours}</div></article></section>
  <section class="split-grid"><div class="glass section"><p class="eyebrow">Services and language</p><h2>What this listing covers</h2><div class="tag-list big-tags">${[...(b.tags || []), ...(b.languages || []).map(l=>`${l} speaking`)].map(t=>`<span>${html(t)}</span>`).join('')}</div></div><div class="glass section"><p class="eyebrow">Source</p><h2>Data lineage</h2><p class="muted">Source file: <code>${html(b.source_file)}</code></p><p class="muted">Source hash: <code>${html(b.source_hash)}</code></p><p class="muted">Last verified: ${html(formatDate(b.last_verified))}</p></div></section>
  ${businessOfferSection(b)}<section class="section glass"><div class="section-head"><div><p class="eyebrow">Related</p><h2>More ${html(b.category)} listings</h2></div><a class="btn small" href="/category/${b.category_slug}/">Open category</a></div><div class="cards">${related.map(miniBusinessCard).join('')}</div></section>`;
  return pageWrap({ title:`${b.name} | PHX Verified`, description:b.description, canonical:`${SITE_URL}/business/${b.id}/`, type:'profile', bodyClass:'profile-page', structuredData:businessStructuredData(b) }, body);
}
...
function businessProfileRendererPage(){
  const body = `<section class="profile-hero glass" id="profileMount"><div><a class="back-link" href="/directory/">← Directory</a><p class="eyebrow">Business profile</p><h1>Loading profile…</h1><p class="hero-text">This route uses the scalable profile renderer for large imports. It loads the matching seeded business from the generated data export.</p></div></section><script>
  ...
  function render(b){
    document.title = b.name + ' | PHX Verified';
    ...
    mount.outerHTML = '<section class="profile-hero glass"><div><a class="back-link" href="/directory/">← Directory</a><p class="eyebrow">'+escapeHtml([b.city,b.category,b.subcategory].filter(Boolean).join(' • '))+'</p><h1>'+escapeHtml(b.name)+'</h1><p class="hero-text">'+escapeHtml(b.description || 'Seeded business profile.')+'</p><div class="badge-row big">'+badges+'</div><div class="action-row">'+(tel?'<a class="btn small" href="tel:'+tel+'">Call</a>':'')+(b.email?'<a class="btn small" href="mailto:'+escapeHtml(b.email)+'">Email</a>':'')+(b.website?'<a class="btn small primary" target="_blank" rel="noopener" href="'+escapeHtml(b.website)+'">Website</a>':'')+'<a class="btn small" href="/request/?business='+escapeHtml(b.id)+'">Request quote</a><a class="btn small" href="/claim/?business='+escapeHtml(b.id)+'">Claim / update</a></div></div><aside class="profile-score"><div class="score huge"><strong>'+escapeHtml(b.verification_score)+'</strong><small>verification score</small></div><div class="profile-mini"><span>'+escapeHtml(priceLabel(b))+'</span><span>'+escapeHtml(b.city || 'Phoenix Metro')+'</span><span>'+escapeHtml(b.license_type || b.category || 'Business')+'</span><span>'+escapeHtml(b.last_verified || '')+'</span></div></aside></section><section class="profile-grid"><article class="glass section"><p class="eyebrow">Source metadata</p><h2>Seeded profile record</h2><div class="policy-list"><div><strong>Identity key</strong><span>'+escapeHtml(b.identity?.primary_key || b.id)+'</span></div><div><strong>Source file</strong><span>'+escapeHtml(b.source_file || '')+'</span></div><div><strong>Address</strong><span>'+escapeHtml([b.address,b.city,b.state,b.zip].filter(Boolean).join(' • '))+'</span></div></div></article><article class="glass section"><p class="eyebrow">Tags</p><h2>Service signals</h2><div class="tag-list">'+(b.tags||[]).slice(0,12).map(t=>'<span>'+escapeHtml(t)+'</span>').join('')+'</div></article></section>';
  }
  const shard = (slug.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0,2) || 'xx');
  fetch('/data/profiles/' + shard + '.json').then(r=>r.json()).then(data=>{
    const b = (data.businesses || []).find(x => x.id === slug);
    if(!b){ mount.innerHTML = '<div><a class="back-link" href="/directory/">← Directory</a><p class="eyebrow">404</p><h1>Business not found.</h1><p class="hero-text">This profile id was not found in the current seed export.</p></div>'; return; }
    render(b);
  }).catch(()=>{ mount.innerHTML = '<div><p class="eyebrow">Error</p><h1>Profile data failed to load.</h1><p class="hero-text">Check /data/businesses.json.</p></div>'; });
</script>`;
  return pageWrap({ title:'Business Profile | PHX Verified', description:'Scalable business profile renderer for PHX Verified seeded business URLs.', canonical:`${SITE_URL}/business-profile/`, bodyClass:'profile-page' }, body);
}


function exposureProducts(){
  const products = [
    { id:'free-seeded-listing', name:'Free seeded listing', audience:'business', price_monthly:0, setup_fee:0, status:'public_supply', includes:['Canonical profile page','City/category discovery','Claim/update packet','Basic search placement'], requirements:['Public business record or submitted owner packet'], sell_motion:'Use as the free supply layer that gives AEs a reason to contact owners.' },
...
function salesPlaybooks(facets, products){
  ...
  return { updated_at:TODAY, products:productNames, ae_openers:[
    'Your business already has a basic PHX Verified profile from public business records. I am reaching out so the actual owner can claim, correct, and improve it before buyers see thin information.',
    ...
    { objection:'I did not ask to be listed.', response:'The free profile is built from public/business seed data. You can claim it, correct it, request suppression, or upgrade it. The goal is one accurate listing, not spam.' },
...
function publicClaimsLedger(report, quality, duplicateClusters, revenue){
  const claims = [
    { claim:'PHX Verified is a seeded Phoenix-area business discovery network.', status:'supported', evidence:`${report.records.published} deduped public business profiles generated from ${report.records.raw} raw seed records.` },
    ...
    { claim:'PHX Verified guarantees leads, rankings, legal compliance, or business results.', status:'blocked', evidence:'Do not claim guaranteed results. Pricing/revenue pages are operator planning models and exposure packages, not outcome guarantees.' }
  ];
...
function pricingPage(products, revenue){
  const body = `<section class="hero glass subhero"><div><p class="eyebrow">Exposure products</p><h1>Turn the seeded marketplace into sellable business exposure.</h1><p class="hero-text">The money path is clear: free seeded supply, owner claims, verified profile upgrades, featured market placement, lead-routing membership, and premium managed growth packs.</p><div class="hero-actions"><a class="btn primary" href="/join/">Claim or upgrade</a><a class="btn" href="/data/exposure-products.json">Open product JSON</a></div></div><aside class="hero-card"><div class="metric"><span>${products.products.length}</span><small>products</small></div><div class="metric"><span>${money(revenue.scenarios.conservative.projected_mrr)}</span><small>modeled MRR</small></div></aside></section><section class="cards">${products.products.map(productCard).join('')}</section>`;
  return pageWrap({ title:'Pricing & Exposure Products | PHX Verified', description:'PHX Verified exposure package model for owner claims, verified profiles, featured placements, lead routing, and managed growth.', canonical:`${SITE_URL}/pricing/`, bodyClass:'pricing-page' }, body);
}
function joinPage(products, activation){
  const body = `<section class="hero glass subhero"><div><p class="eyebrow">Business owner portal</p><h1>Claim, correct, verify, and grow your listing.</h1><p class="hero-text">PHX Verified starts with public seed data, then gives business owners a path to make their profile accurate, trustworthy, and ready for exposure products.</p><div class="hero-actions"><a class="btn primary" href="/claim/">Build claim packet</a><a class="btn" href="/pricing/">View exposure products</a></div></div><aside class="hero-card"><div class="metric"><span>${activation.stats.claim_ready.toLocaleString()}</span><small>claim-ready</small></div><div class="metric"><span>${activation.stats.upgrade_ready.toLocaleString()}</span><small>upgrade-ready</small></div></aside></section><section class="platform-strip"><div class="glass proof-card"><span>1</span><h2>Find profile</h2><p>Open the existing business profile generated from seed data.</p></div><div class="glass proof-card"><span>2</span><h2>Submit proof</h2><p>Use the claim packet to request corrections, ownership review, and verification signals.</p></div><div class="glass proof-card"><span>3</span><h2>Upgrade exposure</h2><p>Move into verified profile, featured market, or lead-routing products once the listing is clean.</p></div></section><section class="cards">${products.products.filter(p=>p.audience==='business').map(productCard).join('')}</section>`;
  return pageWrap({ title:'Join PHX Verified | Claim a Business Listing', description:'Business owner claim, correction, verification, and exposure upgrade workflow for PHX Verified.', canonical:`${SITE_URL}/join/`, bodyClass:'join-page' }, body);
}
function aeCommandPage(territory, playbooks, revenue){
  ...
  return pageWrap({ title:'AE Command Center | PHX Verified', description:'AE sales command center, call queue, playbooks, and activation priorities for PHX Verified.', canonical:`${SITE_URL}/ae-command/`, bodyClass:'ae-command-page' }, body);
}
function activationPage(activation){
  ...
  return pageWrap({ title:'Activation Pipeline | PHX Verified', description:'Commercial activation pipeline for seeded businesses in PHX Verified.', canonical:`${SITE_URL}/activation/`, bodyClass:'activation-page' }, body);
}
function territoriesPage(territory){
  ...
  return pageWrap({ title:'AE Territories | PHX Verified', description:'Generated AE territory plan for PHX Verified city/category marketplace growth.', canonical:`${SITE_URL}/territories/`, bodyClass:'territories-page' }, body);
}
function revenuePage(revenue, products){
  ...
  return pageWrap({ title:'Revenue Readiness | PHX Verified', description:'PHX Verified marketplace revenue readiness model and product packaging.', canonical:`${SITE_URL}/revenue/`, bodyClass:'revenue-page' }, body);
}
...
function productionReadinessPage(readiness, launch){
  ...
  return pageWrap({ title:'Production Readiness | PHX Verified', description:'Production readiness gate for the PHX Verified marketplace package.', canonical:`${SITE_URL}/production-readiness/`, bodyClass:'production-readiness-page' }, body);
}
function claimsLedgerPage(claims){
  ...
  return pageWrap({ title:'Public Claims Ledger | PHX Verified', description:'Supported and blocked public claims for PHX Verified sales and marketplace copy.', canonical:`${SITE_URL}/claims-ledger/`, bodyClass:'claims-ledger-page' }, body);
}
function launchPacketPage(launch){
  ...
  return pageWrap({ title:'Launch Packet | PHX Verified', description:'PHX Verified production launch packet and deployment proof map.', canonical:`${SITE_URL}/launch-packet/`, bodyClass:'launch-packet-page' }, body);
}
...
function offersPage(businesses, facets){
  ...
  return pageWrap({ title:'Offers | PHX Verified', description:'Generated offer marketplace from PHX Verified seeded business pricing and package fields.', canonical:`${SITE_URL}/offers/`, bodyClass:'offers-page' }, body);
}
function mapPage(businesses, facets){
  ...
  return pageWrap({ title:'Map Discovery | PHX Verified', description:'Interactive map-style board generated from PHX Verified seeded business coordinates.', canonical:`${SITE_URL}/map/`, bodyClass:'map-page' }, body);
}
function submitPage(facets){
  ...
  return pageWrap({ title:'Submit Business | PHX Verified', description:'Business submission and seed JSON builder for PHX Verified listings.', canonical:`${SITE_URL}/submit/`, bodyClass:'submit-page' }, body);
}
function requestPage(businesses, facets){
  ...
  return pageWrap({ title:'Request Service | PHX Verified', description:'Buyer service request packet builder for PHX Verified marketplace workflows.', canonical:`${SITE_URL}/request/`, bodyClass:'request-page' }, body);
}
function claimPage(businesses){
  ...
  return pageWrap({ title:'Claim Listing | PHX Verified', description:'Business owner listing claim and update packet builder for PHX Verified.', canonical:`${SITE_URL}/claim/`, bodyClass:'claim-page' }, body);
}
function insightsPage(businesses, facets, report){
  ...
  return pageWrap({ title:'Insights | PHX Verified', description:'Generated marketplace intelligence and data health for PHX Verified.', canonical:`${SITE_URL}/insights/`, bodyClass:'insights-page' }, body);
}
...
function accountWorkbenchPage(opportunities){
  ...
  return pageWrap({ title:'Account Workbench | PHX Verified', description:'Ranked AE account targeting for PHX Verified business activation.', canonical:`${SITE_URL}/accounts/`, bodyClass:'accounts-page' }, body);
}
function pipelinePage(board){
  ...
  return pageWrap({ title:'AE Pipeline | PHX Verified', description:'Generated PHX Verified AE pipeline board for owner activation and sponsor readiness.', canonical:`${SITE_URL}/pipeline/`, bodyClass:'pipeline-page' }, body);
}
function kpiPage(kpi){
  ...
  return pageWrap({ title:'Marketplace KPI | PHX Verified', description:'Marketplace KPIs and money-path readiness metrics for PHX Verified.', canonical:`${SITE_URL}/kpi/`, bodyClass:'kpi-page' }, body);
}
function adminBatchPage(batch){
  ...
  return pageWrap({ title:'Admin Batch Actions | PHX Verified', description:'Batch admin action builder for duplicate suppression, owner verification, and enrichment workflows.', canonical:`${SITE_URL}/admin-batch/`, bodyClass:'admin-batch-page' }, body);
}
function serviceLanesPage(catalog){
  ...
  return pageWrap({ title:'Service Lanes | PHX Verified', description:'PHX Verified service lanes generated from seeded Arizona business data.', canonical:`${SITE_URL}/service-lanes/`, bodyClass:'service-lanes-page' }, body);
}
...
function runtimeStatePage(model){
  ...
  return pageWrap({ title:'Runtime State | PHX Verified', description:'PHX Verified v16 runtime state projection model for claims, leads, AE workflow, and admin actions.', canonical:`${SITE_URL}/runtime-state/`, bodyClass:'runtime-state-page' }, body);
}
function dbContractsPage(bundle){
  ...
  return pageWrap({ title:'Database Contracts | PHX Verified', description:'Database schema and adapter contracts for PHX Verified runtime actions and operational state.', canonical:`${SITE_URL}/db-contracts/`, bodyClass:'db-contracts-page' }, body);
}
function approvalFlowPage(model){
  ...
  return pageWrap({ title:'Approval Flow | PHX Verified', description:'PHX Verified admin approval flow for queued actions and runtime state projection.', canonical:`${SITE_URL}/approval-flow/`, bodyClass:'approval-flow-page' }, body);
}
function backendContractsPage(contracts){
  ...
  return pageWrap({ title:'Backend Contracts | PHX Verified', description:'Upstream-auth-ready runtime action contracts and mutation queue code for PHX Verified.', canonical:`${SITE_URL}/backend/`, bodyClass:'backend-page' }, body);
}
function actionQueuePage(template, opsIndex){
  ...
  return pageWrap({ title:'Action Queue | PHX Verified', description:'PHX Verified action queue model for upstream-auth runtime workflows.', canonical:`${SITE_URL}/action-queue/`, bodyClass:'action-queue-page' }, body);
}
function leadInboxPage(queue){
  ...
  return pageWrap({ title:'Lead Inbox | PHX Verified', description:'Lead inbox routing queue for PHX Verified buyer requests.', canonical:`${SITE_URL}/lead-inbox/`, bodyClass:'lead-inbox-page' }, body);
}
function ownerCrmPage(crm){
  ...
  return pageWrap({ title:'Owner CRM | PHX Verified', description:'Generated owner CRM account index for AE activation.', canonical:`${SITE_URL}/owner-crm/`, bodyClass:'owner-crm-page' }, body);
}
function aeWorkOrdersPage(workOrders){
  ...
  return pageWrap({ title:'AE Work Orders | PHX Verified', description:'Generated AE work orders from PHX Verified seeded marketplace data.', canonical:`${SITE_URL}/ae-work-orders/`, bodyClass:'ae-work-orders-page' }, body);
}

function platformSurfaceLinks(){
  return ['/', '/directory/', '/business/', '/category/', '/city/', '/niche/', '/service-lanes/', '/market/', '/collection/', '/join/', '/pricing/', '/ae-command/', '/accounts/', '/pipeline/', '/activation/', '/territories/', '/kpi/', '/revenue/', '/sales-playbook/', '/trust-network/', '/shortlist/', '/compare/', '/match/', '/lead-routing/', '/deal-desk/', '/offers/', '/map/', '/submit/', '/request/', '/claim/', '/owner-verification/', '/lifecycle/', '/insights/', '/audit/', '/coverage/', '/opportunities/', '/outreach/', '/sponsor/', '/monetization/', '/exports/', '/admin-review/', '/admin-actions/', '/admin-batch/', '/import-health/', '/dry-run/', '/crawl/', '/routing/', '/verification/', '/fraud-defense/', '/duplicates/', '/api/', '/embed/', '/platform/', '/data/', '/operator/', '/production-readiness/', '/claims-ledger/', '/launch-packet/', '/backend/', '/action-queue/', '/lead-inbox/', '/owner-crm/', '/ae-work-orders/'];
}
...
function platformPage(businesses, facets, report){
  ...
  return pageWrap({ title:'Platform Status | PHX Verified', description:'PHX Verified platform build status, generated route inventory, seed file ledger, and proof notes.', canonical:`${SITE_URL}/platform/`, bodyClass:'platform-page' }, body);
}
function dataPage(report){
  ...
  return pageWrap({ title:'Data Pipeline | PHX Verified', description:'Machine-readable platform data exports for PHX Verified Network.', canonical:`${SITE_URL}/data/`, bodyClass:'data-page' }, body);
}
function businessIndexPage(businesses, facets){
  return directoryShell({ businesses, facets, title:'Business Profiles', eyebrow:'Profile index', description:'Browse every generated PHX Verified business profile from one index page.', canonical:`${SITE_URL}/business/` });
}
function categoryIndexPage(businesses, facets){
  ...
  return pageWrap({ title:'Category Index | PHX Verified', description:'Generated category index for PHX Verified business profile hubs.', canonical:`${SITE_URL}/category/`, bodyClass:'category-index-page' }, body);
}
function cityIndexPage(businesses, facets){
  ...
  return pageWrap({ title:'City Index | PHX Verified', description:'Generated city index for PHX Verified Phoenix-area business profile hubs.', canonical:`${SITE_URL}/city/`, bodyClass:'city-index-page' }, body);
}
...
function operatorPage(){
  ...
  return pageWrap({ title:'Operator Import Console | PHX Verified', description:'Operator import console for turning outside scrape exports into PHX Verified seed files.', canonical:`${SITE_URL}/operator/`, bodyClass:'operator-page' }, body, '<script type="module" src="/assets/operator.js"></script>');
}
...
async function main(){
  ...
  await writeFile(writePathFor('/directory/'), directoryShell({ businesses, facets, title:'PHX Verified Business Directory', description:'Search the PHX Verified seeded business marketplace with filters for category, city, verification signals, mobile service, pricing, and tags.' }));
  ...
  for(const cat of facets.categories){
    ...
    await writeFile(writePathFor(route), directoryShell({ businesses:subset, facets, title:`${cat} Businesses`, eyebrow:'Category hub', description:`Browse PHX Verified ${cat.toLowerCase()} listings with profile pages, verification signals, contact actions, and seeded source data.`, canonical:`${SITE_URL}${route}`, routeFilter:{ category:cat } }));
  }
  for(const city of facets.cities){
    const subset = businesses.filter(b => b.city === city);
    const route = `/city/${slugify(city)}/`; routes.push(route);
    await writeFile(writePathFor(route), directoryShell({ businesses:subset, facets, title:`${city} Business Hub`, eyebrow:'City hub', description:`Browse PHX Verified businesses serving ${city}, Arizona with generated profile pages, category filters, and contact actions.`, canonical:`${SITE_URL}${route}`, routeFilter:{ city } }));
  }
  ...
  console.log(`Built PHX Verified platform: ${businesses.length} businesses, ${routes.length + 1} pages, ${loadedFiles.length} seed files.`);
}
