...
function bindProfileTools(){
  $('[data-copy-profile]')?.addEventListener('click', async () => { await navigator.clipboard?.writeText(location.href); toast('Profile link copied'); });
  $('[data-vcard]')?.addEventListener('click', e => {
    const b = JSON.parse(e.currentTarget.dataset.vcard || '{}');
    const lines = ['BEGIN:VCARD','VERSION:3.0',`FN:${b.name || ''}`,`ORG:${b.name || ''}`,b.phone ? `TEL;TYPE=WORK,VOICE:${b.phone}` : '',b.email ? `EMAIL;TYPE=WORK:${b.email}` : '',b.website ? `URL:${b.website}` : '',b.address ? `ADR;TYPE=WORK:;;${b.address};${b.city};${b.state};${b.zip};US` : '',`NOTE:${b.description || 'PHX Verified listing'}`,'END:VCARD'].filter(Boolean).join('\n');
...
function startSky(){
  ...
  function resize(){
    ...
    stars = Array.from({ length: Math.min(180, Math.max(70, Math.floor(innerWidth / 7))) }, () => ({ x:Math.random()*w, y:Math.random()*h, r:Math.random()*1.7+.35, a:Math.random()*.52+.13, vx:(Math.random()-.5)*.06 }));
  }
  function frame(){
    t += .008; ctx.clearRect(0,0,w,h);
    const g = ctx.createRadialGradient(w*.55,h*.10,0,w*.55,h*.10,Math.max(w,h)*.78); g.addColorStop(0,'rgba(139,92,246,.16)'); g.addColorStop(.48,'rgba(247,201,72,.055)'); g.addColorStop(1,'rgba(0,0,0,0)'); ctx.fillStyle = g; ctx.fillRect(0,0,w,h);
    for(const s of stars){ s.x += s.vx; if(s.x < 0) s.x = w; if(s.x > w) s.x = 0; ctx.globalAlpha = s.a + Math.sin(t*3 + s.x*.01)*.08; ctx.fillStyle = 'rgba(255,231,148,.9)'; ctx.beginPath(); ctx.arc(s.x,s.y,s.r,0,Math.PI*2); ctx.fill(); }
    ctx.globalAlpha = 1; requestAnimationFrame(frame);
  }
  addEventListener('resize', resize, { passive:true }); resize(); frame();
}
bindDirectory(); bindProfileTools(); bindShortlistButtons(); bindShortlistPage(); bindComparePage(); startSky();
