(function(){
  ...
  function startHomepageSequence(){
    ...
    introTimer = window.setTimeout(setPreviewMinimized, intro ? 1350 : 0);
  }
  ...
  function mountWorkspaceChat(){
    ...
    const gateConfig = {
      enabled: true,
      password: 'skye-app-template-preview',
      storageKey: 'metraiyux.workspaceGate.skye-app-template-preview-001',
      storage: 'session',
      title: "SKye App Template preview workspace",
      prompt: "Enter the preview access code to unlock this room.",
      chatButtonText: 'Send password through chat'
    };
    window.MetrAIyuxWorkspaceChatConfig = {
      workspaceId: 'skye-app-template-preview-001',
      workspaceSlug: 'client-brand',
      clientName: "SKye App Template",
      appName: "SKye App Template App",
      launcherText: "Client workspace chat",
      operatorName: 'MetrAIyux Operator',
      accent: '#64d6ff',
      apiBase: 'https://relay13-core.graylondonskyes.workers.dev/',
      relayMetadata: {
        account_code: 'CLIENT-BRAND-SKM',
        skye_merit_account: true,
        source_app: 'client-brand',
        source_lane: 'client-workspace-chat',
        relay_bridge: 'relay13-client-workspace'
      },
      accountDisclaimer: "Messages are tied to client workspace account and may be used for support, proof receipts, QA, and follow-up inside this client build lane.",
      accessReply: 'Client, your preview access code is skye-app-template-preview.',
      accessTriggers: ['password', 'access', 'code', 'unlock', 'skye-app-template-preview', 'workspace'],
      passwordGate: workspacePath ? gateConfig : null,
      ...existing,
      accessTriggers: existing.accessTriggers || ['password', 'access', 'code', 'unlock', 'skye-app-template-preview', 'workspace'],
...
(function(){
  if(window.__factorySignatureDeck) return;
  window.__factorySignatureDeck = true;
