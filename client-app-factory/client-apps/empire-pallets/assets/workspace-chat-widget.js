  ...
  async function handleSubmit(event) {
    ...
    win.setTimeout(() => {
      ...
      const replyBody = relayStatus && relayStatus.ok
        ...
        : `Saved locally and queued for ${config.clientName || config.appName} workspace retry. Relay13 is live, but this app did not receive a live receipt yet.`;
  ...
  function mountPasswordGate() {
    ...
    gateNode.innerHTML = `
      <div class="metraiyux-gate-card">
        <h2>${gate.title || "Private workspace"}</h2>
        <p>${gate.prompt || "Enter the workspace password to unlock this screen."}</p>
        <form data-gate-form>
          <label>
            Workspace password
