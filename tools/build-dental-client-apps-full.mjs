...
function renderQueue(){const mount=document.querySelector('[data-intake-queue]'); if(!mount)return; const items=readStore(STORE); mount.innerHTML=items.length?items.map(item=>'<div class="queue-item"><strong>'+escapeHtml(item.name||'New intake')+'</strong><small>'+escapeHtml(item.kind||'Routing')+' · '+escapeHtml(item.urgency||'Routine')+' · '+escapeHtml(item.createdAt)+'</small><small>'+escapeHtml(item.note||'No note')+'</small></div>').join(''):'<div class="queue-item"><strong>No local intake rows yet.</strong><small>Use the form to create a browser-local routing row before opening NorthStar.</small></div>'}
function renderProof(){const mount=document.querySelector('[data-proof-notes]'); if(!mount)return; const items=readStore(PROOF_STORE); mount.innerHTML=items.length?items.map(item=>'<div class="queue-item"><strong>'+escapeHtml(item.subject||'Owner note')+'</strong><small>'+escapeHtml(item.createdAt)+'</small><small>'+escapeHtml(item.note||'No note')+'</small></div>').join(''):'<div class="queue-item"><strong>No owner proof notes saved locally.</strong><small>Use this lane for media approval, listing corrections, and claim proof.</small></div>'}
...
document.addEventListener('submit',event=>{const intake=event.target.closest('[data-intake-form]'); if(intake){event.preventDefault(); const data=Object.fromEntries(new FormData(intake).entries()); const item={...data,createdAt:new Date().toLocaleString(),workspaceSlug:CLIENT.workspaceSlug}; const rows=readStore(STORE); rows.unshift(item); writeStore(STORE,rows); renderQueue(); intake.reset(); const status=document.querySelector('[data-form-status]'); if(status) status.textContent='Saved locally. Open NorthStar to attach this to the workspace.'} const route=event.target.closest('[data-route-form]'); if(route){event.preventDefault(); const data=new FormData(route); location.href='appointments.html?service='+encodeURIComponent(data.get('service')||'')+'&urgency='+encodeURIComponent(data.get('urgency')||'')} const proof=event.target.closest('[data-proof-form]'); if(proof){event.preventDefault(); const data=Object.fromEntries(new FormData(proof).entries()); const rows=readStore(PROOF_STORE); rows.unshift({...data,createdAt:new Date().toLocaleString()}); writeStore(PROOF_STORE,rows); renderProof(); proof.reset(); const status=document.querySelector('[data-proof-status]'); if(status) status.textContent='Owner proof note saved locally.'}})
