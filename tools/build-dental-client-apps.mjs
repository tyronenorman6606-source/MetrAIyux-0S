...
document.addEventListener('submit',event=>{const form=event.target.closest('[data-intake-form]'); if(!form)return; event.preventDefault(); const data=Object.fromEntries(new FormData(form).entries()); const item={...data,createdAt:new Date().toLocaleString(),workspaceSlug:CLIENT.workspaceSlug}; const queue=readQueue(); queue.unshift(item); writeQueue(queue); renderQueue(); form.reset(); const status=document.querySelector('[data-form-status]'); if(status) status.textContent='Saved locally. Open NorthStar to attach it to the workspace.'})
