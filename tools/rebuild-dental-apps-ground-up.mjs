...
function makeTransparentLogo(c, dir) {
  ...
  if (result.status !== 0) throw new Error(`Biltmore transparent logo conversion failed: ${result.stderr?.toString() || result.status}`);
}

function generateHeroVideo(c, dir) {
  ...
  if (result.status !== 0) throw new Error(`Hero video generation failed for ${c.slug}: ${result.stderr?.toString() || result.status}`);
}
...
function sourceSection(c) { return `<section class="section source"><div class="wrap app-grid"><div><div class="eyebrow">Source discipline</div><h2>Real business details and real official media.</h2><p>No fake logos, generated office photos, generated patient scenes, or invented staff. The Valley post is a downstream surface from this app.</p></div><div class="panel"><ul class="checklist"><li>${esc(c.official)}</li><li>${esc(c.source2)}</li><li>${esc(c.address)}</li><li>${esc(c.phone)}</li></ul></div></div></section>`; }
...
for (const c of clients) {
  ...
  for (const dir of dirs) {
    ...
    write(path.join(dir, "APP_PATH_MANIFEST.json"), JSON.stringify({ appSlug: c.slug, appUrl: appUrl(c), valleyUrl: valleyUrl(c), routes: baseRoutes, media: c.media.map(([file, url, alt]) => ({ file, sourceUrl: url, alt })), noGeneratedMedia: true }, null, 2));
    write(path.join(dir, "APP_UPGRADE_PROOF.md"), `# ${c.name} Ground-Up App\n\nBuilt from scratch after removing the prior shell.\n\n- Real official media copied into assets\n- Intro sequence\n- GSAP + Lenis runtime\n- Appointment, route builder, intake, payment, patient center, urgent, workspace, proof, scan, flyer\n- No generated office photos, fake logos, fake doctors, or fake client media\n`);
