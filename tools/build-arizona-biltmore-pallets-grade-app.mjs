...
function addRow(k,row){const rows=read(k); rows.unshift({...row,createdAt:new Date().toLocaleString()}); write(k,rows); renderQueues()}
function renderQueues(){const q=$('[data-intake-queue]'); if(q){const rows=read(INTAKE); $('[data-route-count]')&&($('[data-route-count]').textContent=rows.length+' rows'); q.innerHTML=rows.length?rows.map(r=>'<div class="queue-item"><strong>'+esc(r.name||r.kind||'Dental route')+'</strong><small>'+esc(r.kind)+' · '+esc(r.urgency)+' · '+esc(r.insurance)+' · '+esc(r.createdAt)+'</small><small>'+esc(r.note||'No note')+'</small></div>').join(''):'<div class="queue-item"><strong>No route rows yet.</strong><small>Use the form to create a browser-local handoff row.</small></div>'} const p=$('[data-proof-queue]'); if(p){const rows=read(PROOF); $('[data-proof-count]')&&($('[data-proof-count]').textContent=rows.length+' rows'); p.innerHTML=rows.length?rows.map(r=>'<div class="queue-item"><strong>'+esc(r.kind)+'</strong><small>'+esc(r.createdAt)+'</small><small>'+esc(r.note||'No note')+'</small></div>').join(''):'<div class="queue-item"><strong>No proof rows yet.</strong><small>Owner corrections and approvals save locally in this browser.</small></div>'}}
