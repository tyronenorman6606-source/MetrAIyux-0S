...
const checks = [
  ...
  ['operator session is minted only from shared FS27/SkyGate access', operatorSession.includes('requireAdminAccess') && operatorSession.includes('createOperatorSessionCookie') && operatorPage.includes('hasValidFs27BoundOperatorSession')],
  ['setup page has checklist', setupHtml.includes('setupChecklist') && setupJs.includes('checklistItems')],
  ['setup page keeps token generation out of admin authority', !setupHtml.includes('ADMIN_TOKEN') && !setupJs.includes('ADMIN_TOKEN') && operatorSession.includes('shared_gate_required')],
  ...
  ['diagnostics function requires shared FS27/SkyGate bearer', setupFn.includes('requireAdminReadAccess(event)') || setupFn.includes('requireAdminAccess(event)')],
