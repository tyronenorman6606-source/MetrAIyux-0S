window.SKYECOM = {
  csrfToken: '',
  mountBase() {
    const marker = '/SkyeCommerce';
    return location.pathname === marker || location.pathname.startsWith(`${marker}/`) ? marker : '';
  },
  apiPath(path) {
    const value = String(path || '');
    return value.startsWith('/api/') ? `${this.mountBase()}${value}` : value;
  },
  async ensureCsrfToken() {
    if (this.csrfToken) return this.csrfToken;
    const res = await fetch(this.apiPath('/api/auth/csrf'), { credentials: 'same-origin' });
  ...
  async api(path, options = {}) {
    ...
    const requestPath = this.apiPath(path);
    let res = await fetch(requestPath, { ...options, method, headers, credentials: 'same-origin' });
    let data = await res.json().catch(() => ({}));
    if (res.status === 403 && data.code === 'csrf_invalid') {
      this.csrfToken = '';
      const token = await this.ensureCsrfToken();
      headers.set('X-Skye-CSRF', token);
      res = await fetch(requestPath, { ...options, method, headers, credentials: 'same-origin' });
