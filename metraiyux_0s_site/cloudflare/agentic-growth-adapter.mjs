...
function agenticProviderSandboxEnabled(env = {}) {
  return ['AGENTIC_GROWTH_PROVIDER_RUNTIME_SANDBOX', 'ZERO_OS_PROVIDER_SANDBOX']
    .some((name) => ['1', 'true'].includes(String(env?.[name] || '').toLowerCase()));
}

async function runAgenticProviderAction(env = {}, auth = {}, {provider_id, action, credential_env = {}, payload = {}, usage_lane = ''} = {}) {
  const sandbox = agenticProviderSandboxEnabled(env);
  const runtime = await executeZeroOsAutomationAction({...env, ...credential_env}, {}, {
    provider_id,
    action,
    app_id: 'agentic-growth-layer',
    workspace_id: auth?.customer_id || payload.workspace_id || '',
    ...
    actor: auth?.email || 'agentic-growth-provider-runtime',
    identity: auth || {role: 'operator'}
  }, {
    operator_ok: true
  });
  return {ok: runtime?.response?.ok === true, status: runtime?.status || runtime?.response?.receipt?.http_status || 500, receipt: runtime?.response?.receipt || null, result: runtime?.response?.receipt?.provider_result || {}};
}
...
async function pullSources(payload = {}, env = {}, auth = {}) {
  ...
  if (config.gsc.siteUrl && config.gsc.accessToken) {
    try {
      ...
      const runtime = await runAgenticProviderAction(env, auth, {
        provider_id: 'google-search-console',
        action: 'gsc.search_analytics.query',
        credential_env: {GOOGLE_SEARCH_CONSOLE_ACCESS_TOKEN: config.gsc.accessToken},
        usage_lane: 'agentic_growth.source_pull.gsc',
        payload: {siteUrl: config.gsc.siteUrl, startDate, endDate, dimensions: ['query', 'page'], rowLimit: config.gsc.rowLimit}
      });
      if (!runtime.ok) throw new Error(runtime.receipt?.error || `GSC runtime failed with ${runtime.status}`);
      pulled.gsc = {rows: Array.isArray(runtime.result.rows) ? runtime.result.rows : []};
      receipts.push({source: 'gsc', ok: true, status: runtime.status, rows: pulled.gsc.rows.length, provider_runtime: {receipt_id: runtime.receipt?.id || null, action: runtime.receipt?.action || ''}});
    } catch (error) {
  ...
  if (config.semrush.apiKey && config.semrush.domain) {
    try {
      const runtime = await runAgenticProviderAction(env, auth, {
        ...
        credential_env: {SEMRUSH_API_KEY: config.semrush.apiKey},
        usage_lane: 'agentic_growth.source_pull.semrush',
        payload: {
      ...
      if (!runtime.ok) throw new Error(runtime.receipt?.error || `SEMrush runtime failed with ${runtime.status}`);
      const csv = String(runtime.result.csv || '');
      pulled.semrush = {csv};
      receipts.push({source: 'semrush', ok: true, status: runtime.status, bytes: csv.length, provider_runtime: {receipt_id: runtime.receipt?.id || null, action: runtime.receipt?.action || ''}});
    } catch (error) {
...
// Browser-only MCP chrome is intentionally disabled in the Cloudflare Worker module.
if (false) {
