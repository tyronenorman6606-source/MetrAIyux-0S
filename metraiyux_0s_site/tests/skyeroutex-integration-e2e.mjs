...
async function main() {
  ...
  try {
    ...
    await screenshot(page, 'routex-hub-desktop.png');
    ...
    await screenshot(page, 'routex-v040-hub-desktop.png');
    ...
    await screenshot(page, 'routex-v040-public-desktop.png');

    const routexShell = await page.request.get(`${baseUrl}/SkyeRouteX/index.html`).then((response) => response.text());
    if (!routexShell.includes('./workforce-command-v0.4.0/index.html') || !routexShell.includes('location.replace(target.href)')) {
      throw new Error('RouteX shell does not declare the canonical Workforce Command handoff.');
    }
    await page.goto(`${baseUrl}/SkyeRouteX/workforce-command-v0.4.0/index.html`, { waitUntil: 'domcontentloaded' });
    await expectText(page, 'SkyeRouteX Workforce Command');
    ...
    await screenshot(page, 'routex-app-desktop.png');
    ...
    await screenshot(mobile, 'routex-hub-mobile.png');
    await mobile.close();
    ...
    const report = {
      ...
      assertions: [
        ...
        'RouteX v0.4.0 mounted product UI renders inside the SkyeRouteX folder',
        'Legacy v0.4.0 root path stays quarantined outside the deployed site tree',
        'RouteX shell declares the canonical gate-owned Workforce Command handoff',
