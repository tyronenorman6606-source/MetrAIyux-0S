...
function scoreChunk(query, chunk){
  ...
  ['gray london skyes','government','ae','sales','compliance','staffing','technology','founder','incorporation','legal','resume','cabinet','skygate','fs27','gate','proof surface','live surface','white label','client website','command deck','sovereign infrastructure','metraiyux','privacy','terms','dmca','sms','cookie','refund','enterprise msa','acceptable use','ai terms','solenterprises','legal center','skyes over london'].forEach(p=>{ if(query.toLowerCase().includes(p)&&hay.includes(p)) score+=5; });
  return score;
}
...
async function loadKB(){
  ...
  try{
    const [kbRes, registryRes, legalRes]=await Promise.all([
      fetch('brain/knowledge-base.json'),
      fetch('brain/live-surface-registry.json').catch(()=>null),
      fetch('brain/legal-sync.json').catch(()=>null)
    ]);
    const data=await kbRes.json();
    SURFACE_REGISTRY = registryRes?.ok ? await registryRes.json() : null;
    const legalData = legalRes?.ok ? await legalRes.json() : null;
    KB=[...(data.chunks||[]), ...registryToChunks(SURFACE_REGISTRY), ...(legalData?.chunks||[])];
    const extras = [SURFACE_REGISTRY ? 'live surfaces' : null, legalData ? 'legal center' : null].filter(Boolean);
    status.textContent=`Ready. Loaded ${KB.length} local knowledge chunks${extras.length ? ' including '+extras.join(' and ') : ''}.`;
    renderSources([]);
  }
...
document.getElementById('exampleBrain')?.addEventListener('click',()=>{document.getElementById('brainQuestion').value='Which cabinet owns government contracting readiness, and how should an AE explain the 13-cabinet model to a client?';});
