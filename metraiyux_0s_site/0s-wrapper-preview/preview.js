...
function setLaptopOpen(card, open) {
  const laptop = card?.querySelector(".mcp-macbook-scroll[data-mcp-component='MacbookScroll']");
  const toggle = card?.querySelector("[data-laptop-toggle]");
  const screen = card?.querySelector("[data-laptop-screen]");
  if (!card || !laptop || !toggle || !screen) return;

  card.dataset.open = open ? "true" : "false";
  laptop.dataset.open = open ? "true" : "false";
  toggle.setAttribute("aria-expanded", String(open));
  toggle.setAttribute("aria-label", `${open ? "Close" : "Open"} ${card.querySelector(".window-titlebar span")?.textContent || "MetrAIyux"} laptop`);
...
function syncDock(activeId) {
  document.querySelectorAll("[data-focus-window]").forEach((button) => {
    button.classList.toggle("is-active", button.dataset.focusWindow === activeId);
  });
}

function raiseCard(card) {
  document.querySelectorAll(".mcp-laptop-card").forEach((item) => item.classList.toggle("is-focused", item === card));
  card.style.zIndex = String(++topZ);
  syncDock(card.dataset.window);
}

function focusWindow(id, forceOpen = true) {
  const card = document.querySelector(`.mcp-laptop-card[data-window="${id}"]`);
  if (!card) return;
  raiseCard(card);
  if (forceOpen) setLaptopOpen(card, true);
...
function openCommandOverlay() {
  ...
  document.querySelectorAll("[data-command-toggle]").forEach((button) => button.setAttribute("aria-expanded", "true"));
}

function closeCommandOverlay() {
  commandOverlay?.classList.remove("is-open");
  commandOverlay?.setAttribute("aria-hidden", "true");
  document.querySelectorAll("[data-command-toggle]").forEach((button) => button.setAttribute("aria-expanded", "false"));
}
...
function bindDrag(card) {
  ...
  handle.addEventListener("pointerdown", (event) => {
    if (window.matchMedia("(max-width: 980px)").matches || event.target.closest("button")) return;
    ...
    drag = {
      ...
      maxX: Math.max(0, layer.clientWidth - card.offsetWidth - 18),
      maxY: Math.max(0, layer.scrollHeight - card.offsetHeight - 18)
    };
    raiseCard(card);
  ...
  handle.addEventListener("pointermove", (event) => {
    if (!drag) return;
    const nextX = Math.max(0, Math.min(drag.maxX, drag.x + event.clientX - drag.startX));
    const nextY = Math.max(0, Math.min(drag.maxY, drag.y + event.clientY - drag.startY));
...
document.querySelectorAll(".mcp-laptop-card").forEach((card) => {
  setLaptopOpen(card, card.dataset.open === "true");
  bindDrag(card);

  card.addEventListener("pointerdown", () => raiseCard(card));
  card.querySelector("[data-laptop-toggle]")?.addEventListener("click", () => {
    raiseCard(card);
    setLaptopOpen(card, card.dataset.open !== "true");
  });
  card.querySelector("[data-close-window]")?.addEventListener("click", () => {
    setLaptopOpen(card, false);
  });
});

document.querySelectorAll("[data-focus-window]").forEach((button) => {
  button.addEventListener("click", () => {
    focusWindow(button.dataset.focusWindow, true);
    closeCommandOverlay();
  });
});
...
document.querySelectorAll("[data-command-toggle]").forEach((button) => {
  button.addEventListener("click", () => {
    commandOverlay?.classList.contains("is-open") ? closeCommandOverlay() : openCommandOverlay();
  });
});

document.querySelector("[data-command-close]")?.addEventListener("click", closeCommandOverlay);
...
window.__metraiyuxWrapperPreview = {
  laptopComponent: "MacbookScroll",
  laptopSource: "MCP/skye-design-lab/Compoents/macbook-animation",
  wrapperRestored: true,
  previewOnly: true,
  get openWindows() {
    return [...document.querySelectorAll(".mcp-laptop-card[data-open='true']")].map((card) => card.dataset.window);
  }
};
