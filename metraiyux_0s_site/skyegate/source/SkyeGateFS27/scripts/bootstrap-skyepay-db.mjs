...
async function runQueriesWithSearchPath(queries) {
  const results = await sql.transaction((txn) => [
    txn.query("select set_config('search_path', $1, false)", [DB_SEARCH_PATH]),
...
for (const statement of statements) {
  await runQueriesWithSearchPath([{ text: statement }]);
}

const [tables] = await runQueriesWithSearchPath([{
