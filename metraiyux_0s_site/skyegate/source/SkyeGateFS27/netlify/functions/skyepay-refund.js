...
export default wrap(async (req) => {
  ...
  const orderAmount = Math.max(0, Number(order.amount_setup_cents || 0) + Number(order.amount_recurring_cents || 0));
  const nextPaymentStatus = body.amountCents >= orderAmount ? "refunded" : "partially_refunded";
  await q(
  ...
  await q(
    ...
    [
      ...
      body.reason,
      JSON.stringify({ source: "skyecommerce", skyecommerce: body.skyecommerce })
    ]
  );
