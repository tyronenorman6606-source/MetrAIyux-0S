...
export function buildAuthMeResponse({ user, session, claims }) {
  ...
  return {
    ...
    subject: {
      ...
      password_reset_required: !!(user?.password_reset_required || claims?.password_reset_required)
    },
    session: session ? {
      id: session.id,
      ...
      scope: session.scope || [],
      password_reset_required: !!(user?.password_reset_required || claims?.password_reset_required || session.session_kind === "password_reset"),
