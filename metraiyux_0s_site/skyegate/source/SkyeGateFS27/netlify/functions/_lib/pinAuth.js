...
export async function verifyPinCredential({ gateId, pin }) {
  ...
  const cleanPin = assertValidPin(pin);
  const res = await q(
    `select c.*, u.email, u.email_normalized, u.display_name, u.primary_customer_id, u.default_api_key_id, u.role, u.email_verified_at, u.is_active, u.password_reset_required, u.profile
  ...
  return {
    ...
    user: {
      ...
      default_api_key_id: row.default_api_key_id,
      profile: row.profile
    }
  };
}
...
export async function consumeRecoveryCredential({ gateId, recoveryCode }) {
  ...
  const res = await q(
    `update user_recovery_codes rc
     ...
     returning rc.*, c.gate_id, c.label, u.id as user_id, u.email, u.email_normalized, u.display_name, u.primary_customer_id, u.default_api_key_id, u.role, u.email_verified_at, u.is_active, u.password_reset_required, u.profile`,
