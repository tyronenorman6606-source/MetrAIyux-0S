...
import { getDefaultApiKeyIdForUser, getUserByEmail, getUserPasswordRecord, updateUserLastSeen } from "./_lib/identity.js";
...
function resetSessionTtlSeconds() {
  const parsed = parseInt(process.env.PASSWORD_RESET_SESSION_TTL_SECONDS || "900", 10);
  return Number.isFinite(parsed) ? Math.max(300, Math.min(1800, parsed)) : 900;
}
...
export default wrap(async (req) => {
  ...
  if (user.password_reset_required) {
    const resetSession = await createSession({
      user,
    ...
    await touchSession(resetSession.session_id, { ip: getClientIp(req), userAgent: getUserAgent(req) });
    await audit("auth", "AUTH_LOGIN_PASSWORD_RESET_REQUIRED", `user:${user.id}`, { email: user.email });
    return json(200, {
      ...
      user: buildAuthMeResponse({
        user,
        session: {
          id: resetSession.session_id,
          session_kind: "password_reset",
          customer_id: user.primary_customer_id,
          api_key_id: null,
          scope: resetSession.scope,
          expires_at: resetSession.expires_at,
          created_at: new Date().toISOString()
        },
        claims: resetSession.claims
      }),
      session: {
        token: resetSession.token,
        expires_at: resetSession.expires_at,
        session_id: resetSession.session_id,
        scope: resetSession.scope
      },
      next_step: {
  ...
  return json(200, {
    user: buildAuthMeResponse({ user, session: { id: session.session_id, session_kind: "human", customer_id: user.primary_customer_id, api_key_id: apiKeyId, scope: session.scope, expires_at: session.expires_at, created_at: new Date().toISOString() }, claims: session.claims }),
    session: {
      token: session.token,
