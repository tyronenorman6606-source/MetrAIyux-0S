...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/admin/security/mfa/setup' && request.method === 'POST') {
      ...
      return Response.json({ok:true, device:{id:device.id,label:device.label,status:device.status,account_name,issuer,digits,period,algorithm}, otpauth_uri:uri, qr_svg:qrSvg(uri), secret_base32_once:secret, skye_box_path:'/admin/skyebox-authenticator/index.html'}, {headers:CORS});
    }
