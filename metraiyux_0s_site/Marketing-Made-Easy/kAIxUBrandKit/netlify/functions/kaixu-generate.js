 ...
 * - Keeps the managed gateway lane on the server (Netlify env var)
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Preferred contract: KAIXU_SERVER_LANE. Older deployments may still provide
    // earlier env aliases, so we accept them as compatibility fallbacks.
    const legacyVirtualLaneKey = ["KAIXU", "VIRTUAL", "KEY"].join("_");
    const managedGatewayLane = (
      process.env.KAIXU_SERVER_LANE ||
      process.env[legacyVirtualLaneKey] ||
      process.env.KAIXU_API_KEY ||
      ""
    ).trim();
    if (!managedGatewayLane) {
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({
          ok: false,
          error: "Missing server environment variable: KAIXU_SERVER_LANE",
        }),
      };
    }
    ...
    const resp = await fetch(url, {
      ...
      headers: {
        ...
        "Authorization": `Bearer ${managedGatewayLane}`,
