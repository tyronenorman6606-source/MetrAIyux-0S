...
async function handleAPI(req, res, url){
  ...
  if(method === 'GET' && url.pathname === '/api/legal-review/submissions'){
    ...
    return sendJSON(res, 200, { ok:true, count:rows.length, items:rows.map(summarizeSubmission) });
  }
  ...
  if(method === 'POST' && url.pathname === '/api/legal-review/submit'){
    ...
    const auditEvent = await audit('legal_partner_review_submitted', { submissionId:entry.id, templateId:entry.templateId, riskLevel:entry.riskLevel, servicePlanId:entry.servicePlanId, noGuarantee:true }, req);
    return sendJSON(res, 201, { ok:true, receiptId:entry.id, status:entry.status, auditId:auditEvent.id, submission:summarizeSubmission(entry), boundaries:entry.boundaries, message:'Review request submitted for SovereignDocs triage. This is not legal advice, not a guarantee of partner acceptance, and not an attorney-client relationship with SovereignDocs.' });
  }
  ...
  if(method === 'POST' && legalReviewRouteMatch){
    ...
    const updated = transitionLegalReviewSubmission(rows[index], { status, actor:session.user?.id, note:String(body.note || body.routingNote || ''), partnerId:String(body.partnerId || rows[index].partnerId || 'operator-configured-legal-network'), partnerStatus:body.partnerStatus || null, payload:body.payload || {} });
    rows[index] = updated;
    await writeJSON(LEGAL_REVIEW_SUBMISSIONS_FILE, rows.slice(0,20000));
    ...
    return sendJSON(res, 200, { ok:true, auditId:auditEvent.id, submission:summarizeSubmission(updated), boundary:'SovereignDocs records routing/status only. Partner responsibility, scope, and engagement terms remain external to SovereignDocs.' });
  }
