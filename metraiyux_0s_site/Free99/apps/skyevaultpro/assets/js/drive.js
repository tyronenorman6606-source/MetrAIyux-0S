const VaultPage = {
  ...
  docxMaxUrl(vaultDocId = '') {
    const params = new URLSearchParams({
      source: 'skyevaultpro',
      returnTo: '/Free99/apps/skyevaultpro/drive/index.html'
    });
    if (vaultDocId) params.set('vaultDocId', vaultDocId);
  ...
  bind() {
    ...
    document.querySelector('#new-doc-button')?.addEventListener('click', async () => {
      ...
      window.location.href = this.docxMaxUrl(item.id);
    });
  ...
  openSettings() {
    ...
    if (window.SkyeHosted?.activeUser?.()) window.SkyeHosted.loadProfile?.().catch(() => {});
  },
  ...
  paintQuickAccess() {
    ...
    const links = [
      ...
      { label: 'SkyeDocx', icon: '✦', type: 'link', href: this.docxMaxUrl() },
  ...
  paintFiles() {
    ...
    body.querySelectorAll('[data-edit-item]').forEach((button) => button.addEventListener('click', () => {
      ...
      window.location.href = this.docxMaxUrl(id);
    }));
