# Why the Site Operator Brain Is the Control Layer for MetrAIyux 0S Business Automation
...
- [Site Operator Scorecard](../../nexus/site-operator-scorecard.html) - Evaluate the operator layer by routes and receipts.
- [Business Inbox](../../nexus/business-inbox.html) - Show where signals enter the business.
- [Agent Handoff Receipts](../../nexus/agent-handoff-receipts.html) - Make brain-to-brain movement visible.
- [Admin Automation Brain](../../admin/automation-brain.html) - Open the protected operator layer.
- [Task Lifecycle Ledger](../../crown-os/task-lifecycle-ledger.html) - Track work as it moves through ownership.
...
In practice, this article points to a NEXUS command lane where incoming signals become receipts, cabinet routing, secondary review, and approval prompts. The workflow starts when a signal enters the business: a buyer asks a hard question, a client needs a handoff, a candidate needs placement, a founder needs approval, or a public claim needs evidence. The system should not answer with a vague promise. It should route the signal into the room that owns it.
...
The proof rule for this article is: automation should prove how it routed the work and where human authority entered. That rule matters because serious buyers do not only read for inspiration. They read for risk. They want to know what happens when something goes wrong, who owns the next step, where the record lives, and what the company refuses to automate.
...
The point is not to have a bigger blog. The point is to make every blog post behave like a door into the system. This article should help someone understand site operator control, but it should also move them into the app surface that makes the idea real. That is how the 0S blog becomes useful across the website, sales motion, local brain, and marketing sites.
