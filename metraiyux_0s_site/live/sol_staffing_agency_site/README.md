...
The main site now owns its own proof video assets in `assets/proof/`.

- `assets/proof/sol-staffing-real-workflow.mp4` - browser-recorded MP4 proof
- `assets/proof/sol-staffing-real-workflow.webm` - source WebM browser recording copy
- `assets/proof/sol-staffing-real-workflow-proof.json` - Playwright action path
- `assets/proof/workflow-proof-manifest.json` - video hashes, source package, claims proven, and playback checks

The proof was generated by `tools/record-sol-staffing-proof.mjs` against `/workspaces/MetrAIyux-0S/metraiyux_0s_site/live/sol_staffing_agency_site`. It records employer intake, Skyegate FS27 auth, admin record creation, secure upload, protected brain route guardrail, and local SOL Brain response.
