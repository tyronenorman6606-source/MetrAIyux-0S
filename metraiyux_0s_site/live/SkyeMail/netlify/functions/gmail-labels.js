...
const { getHostedMailbox, zohoApiConfigured, zohoListFolders } = require('./_mailbox-provider');
...
exports.handler = async (event) => {
  try {
    ...
    if (hosted?.provider === 'zoho' && zohoApiConfigured()) {
      ...
      return json(200, { ok: true, mailbox: hosted.mailbox_email, items: folders.items });
    }
