...
- Runnable surface: root standalone mail pages, `suite/` shell, deployed Cloudflare Worker/Netlify-compatible functions, FS27 session exchange, Zoho provider-backed send/read inbox lane, hosted mailbox provisioning endpoints, provider-backed mailbox/runtime lanes, same-folder mail handoff runtime, and the local `build:suite` dist sync lane.
- Proof commands:
...
- Live provider proof: the 2026-05-24 Zoho run sends two real messages through the active hosted-mail provider and reads them back from the provider-backed inbox. The public sanitized receipt is deployed at `https://skyemail-platform.graylondonskyes.workers.dev/proof/live-email-proof.json`.
