...
POST `/api/compliance/manual-checks` — admin/House Command only. Records a manual-government-check proof row with consent/disclosure flags, check types, source, proof reference, optional proof media, audit event, and identity-compliance outbox evidence.
...
Supported non-local/manual provider drivers include `stripe`, `twilio`, `mapbox`, `manual-government-check`, `checkr`, `payment-webhook`, `notification-webhook`, `compliance-webhook`, `skyehands-runtime-webhook`, and `skyehands-runtime-bus`. Missing required credentials fail closed at boot.
