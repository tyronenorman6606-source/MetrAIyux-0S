...
function manualGovernmentComplianceProvider(config = {}) {
  const operatingState = config.COMPLIANCE_OPERATING_STATE || 'AZ';
  const businessMode = config.COMPLIANCE_BUSINESS_MODE || 'llc_admin_assist';
  return {
    driver: 'manual-government-check',
    status: 'manual-compliance',
    note: 'Records consented manual public-record, government-portal, E-Verify/I-9, and proof-vault compliance work. It does not claim CRA status, FBI direct access, or automated background-report furnishing.',
    recordUserAttestation({ db, userId, role, id, now }) {
      const row = {
        id: id('cmp'),
        user_id: userId,
        ...
        status: 'manual_compliance_intake_open',
        checks: [
          'standalone_disclosure_authorization_required',
          'manual_public_record_checklist',
          `${operatingState.toLowerCase()}_business_compliance_lane`,
          'client_or_internal_employer_adjudication_required',
          'proof_vault_required'
        ],
        manual_workflow: {
          operating_state: operatingState,
          business_mode: businessMode,
          cra_position: 'workflow_admin_and_proof_vault_not_background_report_furnisher',
          permitted_use_required: true,
          subject_authorization_required: true,
          adverse_action_packet_required_if_used_to_decline: true
        },
        created_at: now()
      };
      db.compliance_checks.push(row);
    ...
    recordAssignmentAttestation({ db, assignment, id, now }) {
      const row = {
        ...
        provider: this.driver,
        status: 'manual_assignment_compliance_recorded',
        checks: ['assignment_state_machine', 'worksite_scope_review', 'proof_vault_required'],
        manual_workflow: {
          operating_state: operatingState,
          business_mode: businessMode,
          adjudication_owner: 'client_business_or_internal_employer'
        },
        created_at: now()
      };
      db.compliance_checks.push(row);
...
export function createPlatformServices({ env = process.env } = {}) {
  ...
  const complianceDriver = env.IDENTITY_COMPLIANCE_PROVIDER || (localProofAllowed ? 'local-attestation-ledger' : 'manual-government-check');
  ...
  return {
    ...
    compliance: ['local-attestation-ledger', 'none'].includes(complianceDriver) ? localComplianceProvider() : complianceDriver === 'manual-government-check' ? manualGovernmentComplianceProvider(env) : complianceDriver === 'compliance-webhook' ? webhookComplianceProvider(complianceWebhook) : complianceDriver === 'checkr' ? checkrComplianceProvider({ ...env, ...checkrConfig }) : unsupportedProvider('IDENTITY_COMPLIANCE_PROVIDER', complianceDriver),
