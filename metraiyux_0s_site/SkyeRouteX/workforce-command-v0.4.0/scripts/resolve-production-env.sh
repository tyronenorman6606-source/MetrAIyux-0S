...
# Route intelligence is intentionally not faked. Accept the root .env Mapbox alias,
# but production boot should fail if no real token is available.
set_default_from MAPBOX_ACCESS_TOKEN mapbox_api_key MAPBOX_API_KEY SKYEROUTEX_MAPBOX_ACCESS_TOKEN
...
if [ -z "${IDENTITY_COMPLIANCE_PROVIDER:-}" ]; then
  export IDENTITY_COMPLIANCE_PROVIDER=manual-government-check
fi
export COMPLIANCE_OPERATING_STATE="${COMPLIANCE_OPERATING_STATE:-AZ}"
export COMPLIANCE_BUSINESS_MODE="${COMPLIANCE_BUSINESS_MODE:-az_llc_admin_assist}"
