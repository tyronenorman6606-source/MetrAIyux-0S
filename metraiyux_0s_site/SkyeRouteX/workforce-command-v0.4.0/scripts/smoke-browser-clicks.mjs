...
function findRepoRoot(start) {
  ...
  while (current !== path.dirname(current)) {
    if (fs.existsSync(path.join(current, 'package.json')) && fs.existsSync(path.join(current, 'metraiyux_0s_site'))) {
      return current;
    }
...
async function apiJson(method, pathname, body, session) {
  ...
  if (!response.ok) throw new Error(`${method} ${pathname} failed: ${payload.error || text}`);
  return payload;
}

async function attachGateSession(page, { session, user }) {
  await page.evaluate(({ session, user }) => {
    const gate = {
      token: session,
      source: 'local-smoke-gate-session',
      platform_id: 'skyeroutex',
      platform_name: 'SkyeRouteX Workforce Command',
      billing_mode: 'paid_pending_sku',
      usage_lane: 'platform-app',
  ...
  try {
    await page.waitForFunction(expected => JSON.parse(localStorage.getItem('skye_user') || 'null')?.email === expected, user.email, { timeout: 5000 });
  } catch (error) {
    const visible = await page.locator('#session-label').textContent().catch(() => '');
    const toast = await page.locator('#toast').textContent().catch(() => '');
    throw new Error(`gate session label did not update for ${user.email}; visible="${visible}" toast="${toast}"`);
  }
}

async function login(page, email, password) {
  const payload = await apiJson('POST', '/api/auth/login', { email, password });
  await attachGateSession(page, payload);
}

async function signup(_page, { name, email, password, role, company }) {
  await apiJson('POST', '/api/auth/signup', {
    name,
    email,
    password,
    role,
    company_name: company || 'Browser Click Provider Co',
    city: 'Phoenix',
    state: 'Arizona'
  });
}
...
try {
  ...
  await page.waitForSelector('#market-form');
  assert(await page.locator('#login-form').count() === 0 && await page.locator('#signup-form').count() === 0, 'local login/signup forms should not render');
  pass('real_chromium_loaded_gate_owned_app_without_local_login');
  ...
  await page.waitForFunction(() => document.querySelector('#market-select')?.options?.length > 0, null, { timeout: 10000 }).catch(async error => {
    const diagnostic = await page.evaluate(() => ({
      ...
      label: document.querySelector('#session-label')?.textContent || '',
      toast: document.querySelector('#toast')?.textContent || '',
      markets: document.querySelector('#market-select')?.innerHTML || '',
      providerJobs: document.querySelector('#provider-jobs')?.textContent || ''
    }));
    throw new Error(`market select did not populate after create; ${error.message}; diagnostic=${JSON.stringify(diagnostic)}`);
  });
