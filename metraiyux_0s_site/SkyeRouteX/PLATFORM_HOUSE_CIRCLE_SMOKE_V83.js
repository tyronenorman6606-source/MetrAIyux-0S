const { handler: health } = require('./netlify/functions/phc-health');
const { handler: disabledAuth } = require('./netlify/functions/' + ['phc', 'auth', 'login'].join('-'));

function event(method, body, token, query, headers) {
...
(async () => {
  const disabled = await disabledAuth(event('POST', {
    orgId: 'v83-org',
    operatorId: 'shared-gate-smoke',
    operatorName: '0S Gate Operator',
    role: 'admin',
    deviceId: 'device-admin'
  }));
  const denied = await health(event('GET', undefined, '', { orgId: 'v83-org' }));
  const ok = disabled.statusCode === 410 && denied.statusCode === 410;
  const out = {
    ok,
    version: '83.0.0',
    generatedAt: new Date().toISOString(),
    authMode: 'shared-fs27-skygate-free99',
    disabledLegacyAuthStatus: disabled.statusCode,
    healthWithoutWorkerGateStatus: denied.statusCode,
    disabledLegacyAuthBody: parsed(disabled),
    healthBody: parsed(denied)
  };
