...
function truthy(value){ return value === true || ['1','true','yes','y','on','accepted','agree','agreed'].includes(String(value || '').trim().toLowerCase()); }
function legalSource(body){ const nested=body && typeof body.legal_acceptance==='object' ? body.legal_acceptance : {}; const camel=body && typeof body.legalAcceptance==='object' ? body.legalAcceptance : {}; return { ...nested, ...camel, ...(body || {}) }; }
function legalAcceptance(body){
  const input=legalSource(body||{}); const all=truthy(input.legal_acceptance) || truthy(input.accept_legal_pack) || truthy(input.acceptLegalPack) || truthy(input.accepted);
  const flag=(...names)=>all || names.some((name)=>truthy(input[name]));
  return {
    legal_terms_accepted:flag('legal_terms_accepted','termsAccepted','legalTermsAccepted'),
    ...
    legal_acceptance_version:clean(input.legal_version || input.legalVersion || LEGAL_ACCEPTANCE_VERSION),
    legal_acceptance_at:clean(input.accepted_at || input.acceptedAt || ''),
    legal_acceptance_surface:clean(input.acceptance_surface || input.acceptanceSurface || 'skyeroutex-payment-intent')
  };
}
function missingLegalAcceptance(body){ const a=legalAcceptance(body); return ['legal_terms_accepted','arbitration_accepted','payments_policy_accepted','no_outcome_guarantee_accepted','truthful_review_boundary_acknowledged','privacy_policy_accepted'].filter((key)=>a[key]!==true); }
function isCustomerFacing(body){ return truthy(body.customerFacing) || truthy(body.customer_facing) || truthy(body.requireLegalAcceptance) || truthy(body.require_legal_acceptance); }
...
function paymentLegalMeta(body){ const accepted=legalAcceptance(body); return isCustomerFacing(body) ? accepted : { ...accepted, operator_authorized_internal_payment:'true' }; }
function buildStripeIntent(body){ const m=money(body); const params=new URLSearchParams(); const meta=paymentLegalMeta(body); params.set('amount',String(m.amount)); params.set('currency',m.currency.toLowerCase()); params.set('description',compact(body.description || body.memo || 'SkyeRoutex service payment')); params.set('metadata[orgId]', clean(body.orgId || 'default-org')); Object.entries(meta).forEach(([key,value])=>params.set(`metadata[${key}]`, String(value).slice(0,500))); if(clean(body.customerEmail)) params.set('receipt_email',clean(body.customerEmail)); return { amount:m.amount, currency:m.currency, body:params.toString(), contentType:'application/x-www-form-urlencoded' }; }
function buildSquareIntent(body,cfg){ const m=money(body); const meta=paymentLegalMeta(body); return { amount:m.amount, currency:m.currency, body:JSON.stringify({ idempotency_key:idempotencyKey(body), source_id:clean(body.sourceId || body.nonce || 'cnon:card-nonce-required'), amount_money:{ amount:m.amount, currency:m.currency }, location_id:cfg.locationId, note:compact(`${body.description || 'SkyeRoutex service payment'} | ${meta.operator_authorized_internal_payment ? 'operator internal payment' : `Legal Skyes ${meta.legal_acceptance_version}`}`), reference_id:clean(body.orderId || ''), metadata:meta }), contentType:'application/json' }; }
function buildPaypalOrder(body){ const m=money(body); const meta=paymentLegalMeta(body); return { amount:m.amount, currency:m.currency, body:JSON.stringify({ intent:'CAPTURE', purchase_units:[{ reference_id:clean(body.orderId || idempotencyKey(body)), custom_id:clean(body.orderId || idempotencyKey(body)), description:compact(meta.operator_authorized_internal_payment ? 'SkyeRoutex internal operator payment' : `Legal Skyes accepted ${meta.legal_acceptance_version}`), amount:{ currency_code:m.currency, value:(m.amount/100).toFixed(2) } }] }), contentType:'application/json' }; }
async function createPaymentIntent(input, context){
  ...
  if(isCustomerFacing(body)){ const missing=missingLegalAcceptance(body); if(missing.length) return { ok:false, statusCode:403, provider, intentId, error:'Legal Skyes transaction acceptance is required before customer-facing SkyeRouteX payment intent.', code:'LEGAL_ACCEPTANCE_REQUIRED', missing }; }
  ...
  const ledger={ id:intentId, type:'payment_intent', provider, status:'prepared', amount:request.amount, currency:request.currency, orderId:clean(body.orderId), operatorId:clean(context && context.operatorId), legalAcceptance:paymentLegalMeta(body), createdAt:nowISO(), updatedAt:nowISO(), requestPreview:safePreview({ url:request.url, method:request.method, headers:request.headers }) };
...
async function createRefundIntent(input, context){
  ...
  const ledger={ id:refundId, type:'refund_intent', provider, status:'prepared', amount:request.amount, currency:request.currency, paymentRef:clean(body.paymentIntentId || body.paymentId || body.captureId || body.providerPaymentId), operatorId:clean(context && context.operatorId), legalAcceptance:{ operator_authorized_internal_payment:'true', refund_against_existing_payment:'true' }, createdAt:nowISO(), updatedAt:nowISO(), requestPreview:safePreview({ url:request.url, method:request.method, headers:request.headers }) };
  const execution=await providerFetch(request.url,{ method:request.method, headers:request.headers, body:request.body });
