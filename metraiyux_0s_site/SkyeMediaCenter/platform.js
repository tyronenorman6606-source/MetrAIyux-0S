...
const runtimeTiles = [
  ['Asset Intake','/.netlify/functions/media-assets','upload, list, review, execution, dispatch'],
  ['Search Vector','/.netlify/functions/media-search','keyword relevance over stored assets'],
  ['Publish Queue','/.netlify/functions/media-publish','web, social, email target queue'],
  ['File Delivery','/.netlify/functions/media-file','protected draft / public published file serving'],
  ['Stats Pulse','/.netlify/functions/media-stats','operator-only storage and recent upload metrics'],
  ['SkyGate Session','/.netlify/functions/skygate-session','local proof bootstrap plus external bearer verification'],
];
...
async function loadRuntime(){
  ...
  try { const listed = await fetchJson(`${API}/media-assets?action=list`); assets = Array.isArray(listed.assets) ? listed.assets : []; } catch {}
  try { stats = await fetchJson(`${API}/media-stats`); } catch {}
...
async function render(){
  ...
  if(view === 'nerve'){
    ...
    stage.innerHTML = shell('Runtime Spine keeps the actual Netlify Functions visible.', 'runtime spine', runtimeMap(), `<h2>Launch Targets</h2><p>Public intake, operator theater, function smoke, and contract files are linked from the rebuilt shell.</p><a href="./smoke/smoke-proof.mjs">Open Smoke Proof</a>`);
  } else if(view === 'proof'){
...
document.addEventListener('click', async (event)=>{
  ...
  if(cmd === 'search-proof'){
    try{ const res = await fetchJson(`${API}/media-search?q=proof`); toast(`Search pulse returned ${Array.isArray(res.results)?res.results.length:0} hits.`); }catch{ toast('Search pulse could not reach API in this environment.'); }
  }
});
