...
const blocked = await call('/.netlify/functions/media-assets?action=list', { token:'' });
assert(blocked.status === 401, `unauthenticated media list returned ${blocked.status}`);

const session = await parse(await call('/.netlify/functions/skygate-session'));
...
const bootstrap = await call('/.netlify/functions/skygate-session', { method:'POST', body:{ subject:'local-proof' } });
assert(bootstrap.status === 503, `production local bootstrap should be disabled, got ${bootstrap.status}`);

const upload = await parse(await call('/.netlify/functions/media-assets', {
  method:'POST',
  body:{
    action:'upload',
...
const search = await parse(await call('/.netlify/functions/media-search?q=production&type=document'));
assert(search.results?.some((result)=>result.asset?.id === assetId), 'production search did not find uploaded asset');

const review = await parse(await call('/.netlify/functions/media-assets', {
  method:'PUT',
...
const execution = await parse(await call('/.netlify/functions/media-assets', {
  method:'PUT',
...
const dispatch = await parse(await call('/.netlify/functions/media-assets', {
  method:'PUT',
...
const publish = await parse(await call('/.netlify/functions/media-publish', {
  method:'POST',
  body:{ assetId, publishTarget:'web' }
}));
assert(publish.entry?.status === 'published', 'publish did not create a published entry');

const fileBlocked = await call(`/.netlify/functions/media-file?id=${encodeURIComponent(assetId)}`, { token:'' });
assert(fileBlocked.status === 401, `ungated file delivery returned ${fileBlocked.status}`);
const file = await call(`/.netlify/functions/media-file?id=${encodeURIComponent(assetId)}`);
...
const stats = await parse(await call('/.netlify/functions/media-stats'));
assert(stats.totalAssets >= 1 && stats.byStatus.published >= 1, 'production stats did not count published asset');

const archived = await parse(await call(`/.netlify/functions/media-assets?id=${encodeURIComponent(assetId)}`, { method:'DELETE' }));
assert(archived.asset?.status === 'archived', 'archive did not mark asset archived');

const timeline = await parse(await call('/.netlify/functions/media-assets?action=workflow-timeline'));
...
console.log(JSON.stringify({
  ...
  verified:[
    'Cloudflare Worker serves SkyeMediaCenter API routes at .netlify/functions and /api/media aliases',
