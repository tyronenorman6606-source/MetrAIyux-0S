...
FS27 PIN Gate is a separate gate-session recovery lane. It can issue a generated Gate ID, verify a PIN, and send one-time recovery codes through the configured email webhook, but it does not recover this browser-local TOTP vault or decrypt authenticator secrets.
