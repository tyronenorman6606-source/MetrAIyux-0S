...
- `public/upload.html` — Upload Studio with gated audio upload, uploaded audio vault, generated release track lines, and Release Forge handoff.
- `public/player.html` — Music Player with Stream Deck playback and the uploaded audio vault.
- `public/releases.html` — Release room with Artist Nebula, Release Forge, Royalty River, Ops Sequencer, and Live Constellation.
...
- `public/neo-nexus.js` — browser runtime for SkyGate sessions, gated audio uploads, uploaded audio playback, artist creation, release submission, track preview playback, rights attestation, takedown holds, ledger credits, release operations queueing, review/publish/stream/playback reporting, payout completion, analytics reads, and record rendering.
...
- the Upload Studio exists and exposes gated audio upload, generated track lines, uploaded audio vault, and Release Forge handoff
...
- live production R2 credentials, direct browser multipart uploads, transcoding, waveform generation, and CDN delivery
