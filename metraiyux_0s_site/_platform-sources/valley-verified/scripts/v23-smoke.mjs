...
ok(await exists('insights/weekly-review-minutes/index.html'), 'weekly review guide is publicly routed on its publish date');
...
ok(data.version === '23.2.0', 'website-content JSON is v23.2');
...
ok((data.routes || []).includes('/app-builds/'), 'website-content includes app build lane');
...
ok(sitemap.includes('/app-builds/'), 'sitemap includes app build lane');
...
ok(llms.includes('## Client app build lane'), 'llms.txt includes client app build lane context');
